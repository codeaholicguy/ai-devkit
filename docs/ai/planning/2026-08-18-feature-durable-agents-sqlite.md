...
- [x] Foundation: schema migration and connection behavior.
- [x] Store backend: transactional CRUD/ownership behavior.
...
- [x] Use explicit injected `dbPath` values throughout store and integration tests.
...
Schema and readonly connection behavior precede the store rewrite. Row mapping precedes CAS operations. Focused tests precede full gates. `npm ci` and `npm run build` must run before any full gate or commit; both completed during workspace setup.
