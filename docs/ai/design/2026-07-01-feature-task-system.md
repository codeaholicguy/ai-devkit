...
graph TD
  ...
  SVC -->|delegates to| STORE["TaskStore (SPI)"]
  STORE -.->|default impl| SQL["SqliteTaskStore (~/.ai-devkit/tasks.db)"]
...
- **Layering:** CLI and skills call `TaskService`, which delegates to a `TaskStore`
  implementation. Callers never touch storage directly, so a future store can replace
  `SqliteTaskStore` without changing any caller.
...
interface TaskBlocker {
  blockerId: string;   // blk-<YYYYMMDDHHMMSS>-<4 base36>
  text: string;
...
interface TaskEvidence {
  evidenceId: string;  // evd-<YYYYMMDDHHMMSS>-<4 base36>
  command: string | null;
...
interface TaskArtifact {
  artifactId: string;  // art-<YYYYMMDDHHMMSS>-<4 base36>
  path: string;        // reference only — never copied into the task dir
...
interface Task {
  // identity
  taskId: string;          // task-<YYYYMMDDHHMMSS>-<4 base36>; immutable, never reused
  title: string;
  summary: string | null;
...
```ts
interface TaskEvent {
  eventId: string;       // evt-<YYYYMMDDHHMMSS>-<4 base36>
  taskId: string;
  ts: string;            // ISO 8601
...
### `TaskService` (storage-agnostic; consumed by CLI and skills)
...
`--session <s>`. Env: `AIDEVKIT_TASKS_DB`. `<id>` resolves per `resolveTask` above.
...
- **Evidence** records a verification result inline: `command`, `exitCode`, `passed` (required,
  ...
  --summary "all green"`.
- **Artifacts** are **references only** (`path` + optional `kind`/`description`). The store
  never copies files into the task dir, keeping tasks lightweight. To capture text durably, put
  it in evidence `summary`; to point at a file, add an artifact ref.
...
- When `actor` is omitted, the service auto-resolves in order:
  ...
  2. env (`AIDEVKIT_AGENT_ID`, `AIDEVKIT_AGENT_TYPE`, `AIDEVKIT_SESSION_ID`,
     `AIDEVKIT_AGENT_PID` else `process.pid`);
  3. `null` (valid — task still works, just unattributed).
...
Task id format: `task-<YYYYMMDDHHMMSS>-<6 base36>` — sortable by creation, human-readable,
collision-safe. Override the DB path with `--store`, `AIDEVKIT_TASKS_DB`, or the
`SqliteTaskStore` / `DatabaseConnection` constructor. Only `SqliteTaskStore` reads/writes the
database; callers use `TaskService` or the CLI.
...
- **SQLite via the `TaskStore` SPI:** `better-sqlite3` (same as `@ai-devkit/memory`) with WAL +
  `busy_timeout` for safe concurrent local access. The SPI lets a future store replace
  `SqliteTaskStore` with zero caller churn.
- **Stable sortable ids:** lexicographic sort ≈ creation order; no central counter/sequence
  table needed.
...
- **Performance:** indexed `task_id`/`feature`/`status`/`phase` lookups; MVP targets hundreds of
  tasks. (`list` still reads all snapshots in-memory for sort/filter; store-level filtering is a
