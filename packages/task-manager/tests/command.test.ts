...
describe("task command", () => {
  ...
  beforeEach(() => {
    ...
    vi.mocked(readFile).mockResolvedValue(
      JSON.stringify({ tasks: { path: ".ai-devkit/tasks.db" } }),
    );
  ...
  describe("create", () => {
    ...
    it("uses absolute tasks.path from project config as-is", async () => {
      vi.mocked(readFile).mockResolvedValue(
        JSON.stringify({ tasks: { path: "/custom/tasks.db" } }),
      );
      mockTaskService.create.mockResolvedValue(sampleTask());
  ...
  describe("list", () => {
    it("renders a table by default", async () => {
      mockTaskService.list.mockResolvedValue([
        sampleTask(),
        sampleTask({ taskId: SECOND_TASK_ID, title: "Other" }),
      ]);
      const program = createProgram();
      await program.parseAsync(["node", "test", "task", "list"]);

      expect(consoleLogSpy).toHaveBeenCalledWith(
        expect.stringContaining("id\ttitle\tstatus\tphase\tname"),
      );
      expect(consoleLogSpy).toHaveBeenCalledWith(
        expect.stringContaining(`${TASK_ID}\tSample task\topen`),
      );
      expect(consoleLogSpy).toHaveBeenCalledWith(
        expect.stringContaining(`${SECOND_TASK_ID}\tOther\topen`),
      );
    });
    ...
    it("surfaces repository error details when task storage cannot open", async () => {
      const error = new Error("Failed to open task database") as Error & {
        details: Record<string, unknown>;
      };
      error.details = {
        originalError:
          "better-sqlite3 was compiled against NODE_MODULE_VERSION 147; this Node requires 137",
      };
  ...
  describe("show", () => {
    it("resolves the id and prints JSON with --events", async () => {
      ...
      expect(consoleLogSpy).toHaveBeenCalledWith(JSON.stringify({ task, events }, null, 2));
    });
  ...
  describe("phase / status / progress / next", () => {
    ...
    it("sets next step from variadic args", async () => {
      ...
      expect(mockTaskService.setNextStep).toHaveBeenCalledWith(
        task.taskId,
        "write tests",
        expect.any(Object),
      );
  ...
  describe("blocker", () => {
    ...
    it("resolves a blocker by id", async () => {
      ...
      expect(mockTaskService.resolveBlocker).toHaveBeenCalledWith(
        task.taskId,
        BLOCKER_ID,
        expect.any(Object),
      );
  ...
  describe("note", () => {
    it("appends a note from variadic text", async () => {
      ...
      expect(mockTaskService.addNote).toHaveBeenCalledWith(
        task.taskId,
        "quick note",
        expect.any(Object),
      );
  ...
  describe("close", () => {
    it("defaults to completed", async () => {
      ...
      expect(mockTaskService.close).toHaveBeenCalledWith(
        task.taskId,
        "completed",
        expect.any(Object),
      );
