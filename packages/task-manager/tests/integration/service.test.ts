...
describe("TaskService (integration with TaskRepository)", () => {
  ...
  describe("blockers", () => {
    ...
    it("resolveBlocker throws for unknown blocker", async () => {
      const task = await service.create({ title: "T" });
      await expect(service.resolveBlocker(task.taskId, MISSING_BLOCKER_ID)).rejects.toBeInstanceOf(
        TaskResourceNotFoundError,
      );
    });
  });
  ...
  describe("resolveTask", () => {
    ...
    it("throws AmbiguousTaskRefError on ambiguous prefix", async () => {
      ...
      await expect(service.resolveTask("00000000")).rejects.toBeInstanceOf(AmbiguousTaskRefError);
    });
  ...
  describe("addEvent escape hatch", () => {
    ...
    it("rejects unknown event types", async () => {
      const task = await service.create({ title: "T" });
      await expect(service.addEvent(task.taskId, "task.bogus", {})).rejects.toBeInstanceOf(
        UnknownEventTypeError,
      );
    });
  });
