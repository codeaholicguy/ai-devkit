...
describe('resolveCurrentActor', () => {
    ...
    it('ignores a non-numeric AIDEVKIT_AGENT_PID and falls back to process.pid', () => {
    ...
    it('returns null when nothing meaningful resolves', () => {
        // Force process.pid to be undefined by temporarily zeroing pid? Not feasible.
        // Instead verify that with no env we still get an actor carrying only pid.
        const actor = resolveCurrentActor();
        expect(actor).not.toBeNull();
