/**
 * Task system contract types.
 *
 * This file is the LOCKED contract surface. Type names, field names, and the
 * TaskEventType union strings must match the design contract exactly so that
 * consumers (CLI, skills, the tracing worker port) wire up without renames.
 * See docs/ai/design/2026-07-01-feature-task-system.md (+ .CONTRACT.md).
 */
...
// ---------------------------------------------------------------------------
// Task snapshot (task.json — authoritative for reads)
// ---------------------------------------------------------------------------
...
export interface TaskArtifact {
    /** art-<YYYYMMDDHHMMSS>-<4 base36> */
    artifactId: string;
    /** Reference only — never copied into the task directory. */
    path: string;
...
/**
 * Task snapshot. Persisted in `task.json`; authoritative for reads.
 */
export interface Task {
    // identity
    /** task-<YYYYMMDDHHMMSS>-<4 base36>; immutable, never reused. */
    taskId: string;
    title: string;
    summary: string | null;
    ...
    /** Cached count of events (derived from events.jsonl). */
    eventCount: number;
...
// ---------------------------------------------------------------------------
// TaskEvent (one line in events.jsonl — append-only audit trail)
// ---------------------------------------------------------------------------

/**
 * Closed set of event type strings. FROZEN — do not change the literal values.
 */
