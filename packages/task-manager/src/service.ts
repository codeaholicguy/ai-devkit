...
import { makeArtifactId, makeBlockerId, makeEvidenceId, makeEventId, nowIso } from './ids.js';
...
/**
 * TaskService — storage-agnostic. Consumed by the CLI, skills, and the tracing
 * worker port. All methods are async and never touch storage directly; they
 * delegate to the injected `TaskStore` (SPI).
 *
 * Contract surface (method names/signatures are LOCKED):
...
export class TaskService {
    ...
    async create(input: TaskCreateInput): Promise<Task> {
        ...
        const taskId = await this.store.nextTaskId();
    ...
    async resolveTask(ref: TaskRef): Promise<Task | null> {
        if (typeof ref === 'object') {
            if ('taskId' in ref) {
                return this.store.readTask(ref.taskId);
            }
            return this.latestNonTerminalByFeature(ref.feature);
        }

        // Direct id match.
        const direct = await this.store.readTask(ref);
        if (direct) {
            return direct;
        }
        ...
        if (prefixed.length === 1) {
            const match = await this.store.readTask(prefixed[0]!);
            return match;
        }
        if (prefixed.length > 1) {
    ...
    async getEvents(taskId: string, filter: TaskEventsFilter = {}): Promise<TaskEvent[]> {
        ...
        let events = await this.store.readEvents(taskId);
        if (filter.type) {
    ...
    private async refreshCachedCounters(task: Task): Promise<Task> {
        const events = await this.store.readEvents(task.taskId);
