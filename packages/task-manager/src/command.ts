...
import { createTaskService, AmbiguousTaskRefError, isTaskEventType } from "./index.js";
...
function formatActor(
  actor: { agentId?: string; agentType?: string; pid?: number; sessionId?: string } | null,
): string {
  if (!actor) return "-";
  const parts: string[] = [];
...
function renderTask(task: Task): string {
  ...
  if (task.evidence.length) {
    ...
    for (const e of task.evidence) {
      lines.push(
        `    ${e.passed ? "PASS" : "FAIL"} ${e.evidenceId}${e.command ? ` - ${truncate(e.command, 60)}` : ""}`,
      );
    }
  }
...
export function register(command: Command, runtime: AiDevkitRuntime): void {
  ...
  addAttributionFlags(
    ...
    withErrorHandler("update task", async (id: string, opts) => {
      ...
      const updated = await service.update(resolved.taskId, patch, {
        actor: actorFromOptions(opts),
      });
  ...
  addAttributionFlags(
    command.command("phase <id> <phase>").description("Set the lifecycle phase"),
  ).action(
    withErrorHandler("set task phase", async (id: string, phase: string, opts) => {
      const service = await createService(runtime, opts.dbPath);
      const resolved = await resolveOrError(runtime, service, id);
      if (!resolved) return;
      const updated = await service.setPhase(resolved.taskId, phase, {
        actor: actorFromOptions(opts),
      });
      output(updated, opts.json);
    }),
  ...
  addAttributionFlags(
    command.command("status <id> <status>").description(`Set status (${VALID_STATUSES.join("|")})`),
  ).action(
  ...
  addAttributionFlags(
    ...
    withErrorHandler(
      "set task next step",
      async (id: string, stepParts: string[] | undefined, opts) => {
        const service = await createService(runtime, opts.dbPath);
        const resolved = await resolveOrError(runtime, service, id);
        if (!resolved) return;
  ...
  addAttributionFlags(
    ...
    withErrorHandler(
      "manage blocker",
      async (id: string, action: string, rest: string[] | undefined, opts) => {
        const service = await createService(runtime, opts.dbPath);
        const resolved = await resolveOrError(runtime, service, id);
        if (!resolved) return;
        const args = rest ?? [];
  ...
  addAttributionFlags(
    command.command("note <id> [text...]").description("Append a note (event-only)"),
  ).action(
