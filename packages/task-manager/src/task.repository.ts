...
import { getDatabase, DEFAULT_DB_PATH, type DatabaseConnection } from "./database/connection.js";
    ...
    try {
      this.db().execute(
        `INSERT OR REPLACE INTO tasks (task_id, snapshot, name, status, phase, created_at, updated_at)
                 VALUES (?, ?, ?, ?, ?, ?, ?)`,
        [task.taskId, snapshot, task.name, task.status, task.phase, task.createdAt, task.updatedAt],
      );
    } catch (error) {
