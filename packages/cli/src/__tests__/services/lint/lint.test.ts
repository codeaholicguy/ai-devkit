...
describe("lint service", () => {
  ...
  it("passes with warning when branch exists but no dedicated worktree", () => {
    const report = runLintChecks({ feature: "feature-sample" }, undefined, {
      cwd: () => "/repo",
      existsSync: () => true,
  ...
  it("fails when feature branch does not exist", () => {
    const report = runLintChecks({ feature: "sample" }, undefined, {
      cwd: () => "/repo",
      existsSync: () => true,
      execFileSync: (_file: string, args: readonly string[]) => {
        const cmd = args.join(" ");
  ...
  it("uses configured phases for base and feature docs", () => {
    ...
    const report = runLintChecks({ feature: "sample" }, undefined, ["requirements", "deployment"], {
      cwd: () => "/repo",
      existsSync: (p: string) => existingPaths.has(p),
  ...
  it("fails fast for invalid feature names", () => {
    ...
    expect(report.exitCode).toBe(1);
    expect(
      report.checks.some((check) => check.id === "feature-name" && check.level === "miss"),
    ).toBe(true);
  });
