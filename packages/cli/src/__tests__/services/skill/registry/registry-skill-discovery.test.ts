...
describe("local registry filesystem boundary", () => {
  ...
  it("rejects skill and metadata symlinks that escape the registry", async () => {
    ...
    await expect(resolveContainedSkill("test/skills", root, "escape-skill")).rejects.toThrow(
      /outside/i,
    );
    ...
    await expect(resolveContainedSkill("test/skills", root, "metadata-escape")).rejects.toThrow(
      /outside/i,
    );
  });
  ...
  it("rejects oversized metadata on an explicit skill install path", async () => {
    ...
    await expect(resolveContainedSkill("test/skills", root, "safe-skill")).rejects.toThrow(
      /too large/i,
    );
  });
});
