...
describe("plugin manifest service", () => {
  describe("validatePluginManifest", () => {
    ...
    it("rejects packages without aiDevkit command manifests", () => {
      expect(() => validatePluginManifest("@ai-devkit/bad-plugin", {}, new Set())).toThrow(
        "Plugin @ai-devkit/bad-plugin must define package.json aiDevkit.commands.",
      );
    });
    ...
    it("rejects duplicate command names within the same plugin", () => {
      ...
      ).toThrow('Plugin @ai-devkit/bad-plugin declares duplicate command "memory-dashboard".');
    });
    ...
    it("rejects command names that conflict with built-in commands", () => {
      expect(() =>
        validatePluginManifest(
          ...
          {
            aiDevkit: {
              commands: [{ name: "memory", entry: "./dist/command.js" }],
            },
          },
          new Set(["memory"]),
        ),
      ).toThrow('Plugin @ai-devkit/bad-plugin command "memory" conflicts with a built-in command.');
    });
      ...
      for (const name of invalidNames) {
        expect(() =>
          validatePluginManifest(
            ...
            {
              aiDevkit: {
                commands: [{ name, entry: "./dist/command.js" }],
              },
            },
            new Set(),
          ),
        ).toThrow(
          `Plugin @ai-devkit/bad-plugin command "${name}" must use lowercase letters, numbers, and hyphens only, and start with a letter.`,
        );
      }
    });
    ...
    it("rejects non-JavaScript command entrypoints", () => {
      expect(() =>
        validatePluginManifest(
          ...
          {
            aiDevkit: {
              commands: [{ name: "memory-dashboard", entry: "./src/command.ts" }],
            },
          },
          new Set(),
        ),
      ).toThrow(
        'Plugin @ai-devkit/bad-plugin command "memory-dashboard" must point to a JavaScript entrypoint.',
      );
    });
  });
  ...
  describe("resolvePluginCommandEntry", () => {
    it("resolves command entry paths inside the plugin package root", () => {
      ...
      expect(entry).toBe(
        path.resolve(
          "/home/test/.ai-devkit/npm/node_modules/@ai-devkit/memory-dashboard/dist/command.js",
        ),
      );
    });
