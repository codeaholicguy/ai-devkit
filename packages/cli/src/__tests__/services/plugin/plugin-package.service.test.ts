...
describe("plugin package service", () => {
  ...
  it("wraps npm install failures with plugin context", async () => {
    ...
    await expect(service.install("@ai-devkit/memory-dashboard")).rejects.toThrow(
      "Failed to install plugin package @ai-devkit/memory-dashboard: network down",
    );
  });
  ...
  it("wraps npm uninstall failures with plugin context", async () => {
    ...
    await expect(service.uninstall("@ai-devkit/memory-dashboard")).rejects.toThrow(
      "Failed to uninstall plugin package @ai-devkit/memory-dashboard: permission denied",
    );
  });
  ...
  it("rejects empty package names before running npm", async () => {
    ...
    await expect(service.install("   ")).rejects.toThrow(
      "Plugin package name must be a non-empty npm package name.",
    );
  ...
  it("rejects package names that look like local paths before running npm", async () => {
    ...
    await expect(service.install("./memory-dashboard")).rejects.toThrow(
  ...
  it("rejects package specs with explicit versions before running npm", async () => {
    ...
    await expect(service.install("@ai-devkit/memory-dashboard@1.0.0")).rejects.toThrow(
      "Only npm package names are supported for plugins.",
    );
    await expect(service.install("memory-dashboard@1.0.0")).rejects.toThrow(
      "Only npm package names are supported for plugins.",
    );
