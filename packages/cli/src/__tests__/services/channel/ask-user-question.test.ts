...
describe("AskUserQuestionService", () => {
  ...
  it("sends digit key (optionIdx + 1) when option is tapped", async () => {
    ...
    const kb = channel.sendInlineKeyboard.mock.calls[0][2] as Array<
      Array<{ callbackData: string }>
    >;
  ...
  it("ignores callback with mismatched chatId", async () => {
    ...
    const data = (
      channel.sendInlineKeyboard.mock.calls[0][2] as Array<Array<{ callbackData: string }>>
    )[0][0].callbackData;
