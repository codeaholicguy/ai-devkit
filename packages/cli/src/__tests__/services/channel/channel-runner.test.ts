...
describe("startOutputPolling — agent requests", () => {
  ...
  it("does not send when chatIdRef.value is null", async () => {
    ...
    const interval = startOutputPolling(
      telegram as never,
  ...
  it("sends again when agent-request file is overwritten with a new timestamp", async () => {
    ...
    writeAgentRequest(
      homeDir,
      makeRequest({ toolInput: { command: "echo first" }, timestamp: "2026-06-29T00:00:01.000Z" }),
    );
    await vi.advanceTimersByTimeAsync(2100); // tick 1 — sends first

    writeAgentRequest(
      homeDir,
      makeRequest({ toolInput: { command: "echo second" }, timestamp: "2026-06-29T00:00:02.000Z" }),
    );
  ...
  it("conversation messages are sent before agent-request message in the same tick", async () => {
    ...
    const interval = startOutputPolling(telegram as never, adapter as never, agent, chatIdRef, {
      homeDir,
    });
  ...
  it("routes AskUserQuestion single-select payload to inline-keyboard service", async () => {
      ...
      questions: [
        {
          ...
          options: [
            {
              label: "Continue the bug fix",
              description: "Pick up where we left off on fixing the bug",
            },
    ...
    const askQuestion = new AskUserQuestionService(
      telegram as never,
    ...
    writeAgentRequest(
      homeDir,
      makeRequest({ toolName: "AskUserQuestion", toolInput: singleSelectInput }),
    );
    await vi.advanceTimersByTimeAsync(2100);
    clearInterval(interval);
  ...
  it("routes AskUserQuestion multi-select payload with Skip-only keyboard", async () => {
    ...
    writeAgentRequest(
      homeDir,
      makeRequest({ toolName: "AskUserQuestion", toolInput: multiSelectInput }),
    );
    await vi.advanceTimersByTimeAsync(2100);
    clearInterval(interval);
