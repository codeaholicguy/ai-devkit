...
describe("ChannelService", () => {
  ...
  it("rejects duplicate Telegram tokens across different channel names", () => {
    expect(() =>
      service.assertUniqueTelegramToken(
        {
          channels: {
    ...
    ).toThrow('Telegram bot token is already configured for channel "personal".');
  });
  ...
  it("uses the explicit channel name for start when provided", () => {
    expect(
      service.resolveStartChannelName(
        {
          channels: {
            telegram: personalEntry,
  ...
  it("requires an explicit channel name for start when multiple Telegram channels exist", () => {
    ...
    ).toThrow("Multiple Telegram channels configured. Specify one: personal, work");
  });
  ...
  it("fails start resolution when no Telegram channels exist", () => {
    ...
    ).toThrow('No Telegram channel configured. Run "ai-devkit channel connect telegram" first.');
  });
  ...
  it("stores multiple bridge entries by channel name", async () => {
    ...
    expect(await service.getLiveBridgeByChannel("team-slack")).toEqual(
  ...
  it("prunes stale bridge entries", async () => {
    ...
    expect(live).toEqual([expect.objectContaining({ channelName: "personal", bridgePid: 200 })]);
  ...
  it("starts a daemon bridge by spawning a detached child and recording its pid", async () => {
    ...
    expect(await service.getLiveBridgeByChannel("personal")).toEqual(
      expect.objectContaining({
        channelName: "personal",
        bridgePid: 300,
        logPath: path.join(tmpDir, "channel-logs", "personal.log"),
  ...
  it("requires a channel name when multiple live bridges exist", async () => {
    ...
    await expect(service.stopBridge()).rejects.toThrow(
      "Multiple channel bridges are running. Specify one: personal, work",
    );
