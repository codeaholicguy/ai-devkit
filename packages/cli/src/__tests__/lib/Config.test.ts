...
describe('ConfigManager', () => {
  ...
  describe('getTasksDbPath', () => {
    it('returns undefined when config does not exist', async () => {
      (mockFs.pathExists as any).mockResolvedValue(false);

      const result = await configManager.getTasksDbPath();

      expect(result).toBeUndefined();
    });

    it('returns undefined when tasks.path is missing', async () => {
      (mockFs.pathExists as any).mockResolvedValue(true);
      (mockFs.readJson as any).mockResolvedValue({
        version: '1.0.0',
        environments: [],
    ...
    it('returns undefined when tasks.path is blank or invalid', async () => {
      (mockFs.pathExists as any).mockResolvedValue(true);
      (mockFs.readJson as any).mockResolvedValue({
        version: '1.0.0',
        environments: [],
      ...
      await expect(configManager.getTasksDbPath()).resolves.toBeUndefined();
    ...
    it('returns absolute tasks.path as-is', async () => {
      (mockFs.pathExists as any).mockResolvedValue(true);
      (mockFs.readJson as any).mockResolvedValue({
        version: '1.0.0',
        environments: [],
        phases: [],
        tasks: { path: '/custom/tasks.db' },
        createdAt: '2024-01-01T00:00:00.000Z',
      });

      const result = await configManager.getTasksDbPath();

      expect(result).toBe('/custom/tasks.db');
      expect(mockPath.isAbsolute).toHaveBeenCalledWith('/custom/tasks.db');
    });

    it('resolves relative tasks.path from the config directory', async () => {
      (mockFs.pathExists as any).mockResolvedValue(true);
      (mockFs.readJson as any).mockResolvedValue({
        version: '1.0.0',
        environments: [],
        phases: [],
        tasks: { path: '.ai-devkit/project-tasks.db' },
        createdAt: '2024-01-01T00:00:00.000Z',
      });
      ...
      expect(mockPath.dirname).toHaveBeenCalledWith('/test/dir/.ai-devkit.json');
      expect(mockPath.resolve).toHaveBeenCalledWith('/test/dir', '.ai-devkit/project-tasks.db');
      expect(result).toBe('/test/dir/.ai-devkit/project-tasks.db');
    });
  });
});
