...
describe("ConfigManager", () => {
  ...
  describe("getAgentRuntimeProvider", () => {
    ...
    it("ignores project-level runtime config because agent runtime is global-only", async () => {
      (mockFs.pathExists as any).mockImplementation(
        async (configPath: string) => configPath === "/test/dir/.ai-devkit.json",
      );
      (mockFs.readJson as any).mockResolvedValue({
        version: "1.0.0",
        environments: [],
  ...
  describe("create", () => {
    it("should create config with default values", async () => {
      ...
      expect(mockFs.writeJson).toHaveBeenCalledWith("/test/dir/.ai-devkit.json", expectedConfig, {
        spaces: 2,
      });
  ...
  describe("removeSkillRegistry", () => {
    ...
    it("rejects removal when project config is missing", async () => {
      (mockFs.pathExists as any).mockResolvedValue(false);
      await expect(configManager.removeSkillRegistry("target/skills")).rejects.toThrow(
        "Config file not found",
      );
      expect(mockFs.writeJson).not.toHaveBeenCalled();
    });
