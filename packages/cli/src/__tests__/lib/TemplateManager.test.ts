...
describe("TemplateManager", () => {
  ...
  describe("setupSingleEnvironment", () => {
    ...
    it("should handle cursor environment with special files", async () => {
      ...
      expect(result).toContain(
        path.join(templateManager["targetDir"], ".cursor", "rules", "rule1.md"),
      );
      expect(result).toContain(
        path.join(templateManager["targetDir"], ".cursor", "rules", "rule2.toml"),
      );
    });
    ...
    it("should handle errors and rethrow them", async () => {
      ...
      await expect((templateManager as any).setupSingleEnvironment(env)).rejects.toThrow(
        "Test error",
      );
  ...
  describe("setupMultipleEnvironments", () => {
    it("should setup multiple environments successfully", async () => {
      ...
      mockGetEnvironment.mockReturnValueOnce(cursorEnv).mockReturnValueOnce(geminiEnv);
    ...
    it("should skip invalid environments and continue with valid ones", async () => {
      ...
      expect(result).toEqual(["/path/to/cursor/file1", "/path/to/gemini/file1"]);
    });
    ...
    it("should throw error when setupSingleEnvironment fails", async () => {
      ...
      await expect(templateManager.setupMultipleEnvironments(envCodes)).rejects.toThrow(
        "Setup failed",
      );

      expect(mockUi.error).toHaveBeenCalledWith(
        "Error setting up environment 'Cursor': Setup failed",
      );
    });
  });
