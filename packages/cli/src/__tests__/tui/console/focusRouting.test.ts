...
describe("console focus routing", () => {
  ...
  it("keeps message input routing available from list and detail focus", () => {
    ...
    expect(resolve({ focus: "detail", input: "i", hasSelectedAgent: false })).toEqual({
      type: "noop",
    });
  });
  ...
  it("routes lowercase p to pin toggling only from list focus", () => {
    ...
    expect(resolve({ focus: "list", input: "p", hasSelectedAgent: false })).toEqual({
      type: "noop",
    });
  });
  ...
  it("clears an active list filter with escape", () => {
    expect(resolve({ key: { escape: true }, filterActive: true })).toEqual({
      type: "clear-filter",
    });
  ...
  it("routes every key as a no-op while the filter input owns focus", () => {
    ...
    expect(resolve({ focus: "filter" as ConsoleFocus, key: { escape: true } })).toEqual({
      type: "noop",
    });
  });
});
