...
describe("MemoryListPane helpers", () => {
  ...
  it("classifies loading, error, empty, and item states", () => {
    expect(getMemoryListDisplayState({ isLoading: true, error: null, rowCount: 0 })).toBe(
      "loading",
    );
    expect(getMemoryListDisplayState({ isLoading: false, error: "db failed", rowCount: 0 })).toBe(
      "error",
    );
  ...
  it("returns status text for non-item display states", () => {
    expect(getMemoryListStatusText({ displayState: "loading", error: null, width: 80 })).toBe(
      "loading...",
    );
    expect(
      getMemoryListStatusText({ displayState: "error", error: "x".repeat(20), width: 10 }),
    ).toBe("xxxxxxx...");
    expect(getMemoryListStatusText({ displayState: "error", error: null, width: 80 })).toBe(
      "Could not load memory.",
    );
    expect(getMemoryListStatusText({ displayState: "empty", error: null, width: 80 })).toBe(
      "No memory items yet.",
    );
