...
describe("AgentListPane helpers", () => {
  it("uses a compact ASCII remote marker for connected agents", () => {
    expect(
      getAgentChannelMarker({ channelName: "telegram", channelType: "telegram", bridgePid: 42 }),
    ).toBe("remote");
  });
  ...
  it("renders filtered counts, query state, and remote channel markers", async () => {
    ...
    const instance = render(
      React.createElement(AgentListPane, {
        agents,
        ...
        filterText: "agent",
        channelStatuses: {
          "Alpha Agent": { channelName: "telegram", channelType: "telegram", bridgePid: 42 },
        },
      }),
  ...
  it("selects the first recency-ordered pin and falls back to the first agent", () => {
    ...
    expect(selectInitialAgentName([agent("fallback", false, "2026-08-16T01:00:00Z")])).toBe(
      "fallback",
    );
