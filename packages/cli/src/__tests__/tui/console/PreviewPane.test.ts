...
describe("PreviewPane helpers", () => {
  it("uses success tone when the selected agent has channel status", () => {
    expect(
      getPreviewPanelTone({ channelName: "telegram", channelType: "telegram", bridgePid: 42 }),
    ).toBe("success");
  });
  ...
  it("renders conversation turns using the agent detail format", () => {
    ...
    expect(output).toMatch(/user:\s+first question\s+assistant:\s+first answer\s+with detail/u);
  ...
  it("reclaims the activity row for conversation content when the agent stops running", () => {
    ...
    expect(waiting).toMatch(/line two\s+line three\s+line four/u);
    expect(waiting).not.toContain("working");
  });

  it.each([AgentStatus.WAITING, AgentStatus.IDLE, AgentStatus.UNKNOWN])(
    "hides the indicator for %s agents",
    (status) => {
      const agent = {
        name: "preview-test",
        type: "codex",
  ...
  it("renders Markdown message bodies without source punctuation", () => {
    ...
    expect(output).toMatch(
      /assistant:\s+Heading\s+Use bold and docs \(https:\/\/example\.com\)\./u,
    );
  ...
  it("does not reparse Markdown or rebuild laid-out rows when only scroll offset changes", async () => {
    ...
    const preview = (scrollOffset: number) =>
      React.createElement(PreviewPane, {
        agent,
