...
describe("TerminalUI", () => {
  ...
  describe("info()", () => {
    it("should display blue info message with ℹ symbol", () => {
      ...
      expect(consoleLogSpy).toHaveBeenCalledWith("[BLUE]ℹ[/BLUE]", "Test info message");
    });

    it("should sanitize ANSI escape codes", () => {
      ui.info("Test \x1b[31mwith color\x1b[0m message");

      expect(consoleLogSpy).toHaveBeenCalledWith("[BLUE]ℹ[/BLUE]", "Test with color message");
    });
  ...
  describe("success()", () => {
    it("should display green success message with ✔ symbol", () => {
      ...
      expect(consoleLogSpy).toHaveBeenCalledTimes(1);
      expect(consoleLogSpy).toHaveBeenCalledWith("[GREEN]✔[/GREEN]", "Test success message");
    });

    it("should sanitize ANSI escape codes", () => {
      ui.success("Test \x1b[31mwith color\x1b[0m message");

      expect(consoleLogSpy).toHaveBeenCalledWith("[GREEN]✔[/GREEN]", "Test with color message");
    });
  });
  ...
  describe("warning()", () => {
    it("should display yellow warning message with ⚠ symbol", () => {
      ...
      expect(consoleLogSpy).toHaveBeenCalledTimes(1);
      expect(consoleLogSpy).toHaveBeenCalledWith("[YELLOW]⚠[/YELLOW]", "Test warning message");
    });

    it("should sanitize ANSI escape codes", () => {
      ui.warning("Test \x1b[31mwith color\x1b[0m message");

      expect(consoleLogSpy).toHaveBeenCalledWith("[YELLOW]⚠[/YELLOW]", "Test with color message");
    });
  });
  ...
  describe("error()", () => {
    it("should display red error message with ✖ symbol", () => {
      ...
      expect(consoleErrorSpy).toHaveBeenCalledWith("[RED]✖[/RED]", "Test error message");
    });
    ...
    it("should sanitize ANSI escape codes", () => {
      ...
      expect(consoleErrorSpy).toHaveBeenCalledWith("[RED]✖[/RED]", "Test with color message");
    });
  });
  ...
  describe("summary()", () => {
    ...
    it("should use custom title when provided", () => {
      ui.summary({
        ...
        items: [{ type: "success", count: 1, label: "done" }],
      });
    ...
    it("should display details section when provided", () => {
      ui.summary({
        items: [{ type: "error", count: 1, label: "failed" }],
        details: {
          title: "Errors",
          items: [{ message: "Error 1", tip: "Fix this" }, { message: "Error 2" }],
        },
      });
  ...
  describe("Edge cases", () => {
    it("should handle very long messages", () => {
      ...
      expect(consoleLogSpy).toHaveBeenCalledWith("[BLUE]ℹ[/BLUE]", longMessage);
    });

    it("should handle messages with newlines", () => {
      ui.info("Line 1\nLine 2\nLine 3");

      expect(consoleLogSpy).toHaveBeenCalledWith("[BLUE]ℹ[/BLUE]", "Line 1\nLine 2\nLine 3");
    });
    ...
    it("should handle multiple ANSI escape codes", () => {
      ...
      expect(consoleLogSpy).toHaveBeenCalledWith("[BLUE]ℹ[/BLUE]", "Red Green Blue");
    });
  });
