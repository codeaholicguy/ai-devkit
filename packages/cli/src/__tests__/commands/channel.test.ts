...
const mockChannelService = {
  ...
  resolveStartChannelName: vi.fn(
    (config: any, name?: string) => name ?? Object.keys(config.channels)[0],
  ),
...
vi.mock(
  ...
  () => ({
    ChannelManager: vi.fn(function () {
      return mockChannelManager;
    }),
    ChannelConfigRepository: vi.fn(function () {
      return mockConfigStore;
    }),
    TelegramAdapter: vi.fn(function () {
...
describe("startOutputPolling", () => {
  ...
  beforeEach(() => {
    ...
    mockChannelService.resolveConnectChannelName.mockImplementation(
      (name?: string) => name ?? "telegram",
    );
    mockChannelService.resolveStartChannelName.mockImplementation(
      (config: any, name?: string) => name ?? Object.keys(config.channels)[0],
    );
...
describe("channel command", () => {
  ...
  it("debugs the failed Slack validation stage without logging credentials", async () => {
    ...
    mockValidateSlackAppToken.mockRejectedValueOnce(
      new Error(`socket_mode_disabled for ${appToken} and ${botToken}`),
    );
    const program = new Command();
    registerChannelCommand(program);
  ...
  it("records the bridge before starting the channel manager", async () => {
    ...
    expect(mockChannelService.registerBridge).toHaveBeenCalledWith(
      expect.objectContaining({
        channelName: "personal",
        channelType: "telegram",
        agentName: "codex-main",
  ...
  it("starts a daemon bridge without resolving the agent in the parent process", async () => {
    ...
    expect(mockChannelService.startDaemonBridge).toHaveBeenCalledWith(
      expect.objectContaining({
        channelName: "personal",
        channelType: "telegram",
        agentName: "codex-main",
    ...
    expect(daemonInput.args).toEqual(
      expect.arrayContaining([expect.stringContaining("channel-daemon.ts")]),
    );
