...
vi.mock('@ai-devkit/task-manager', () => ({
    // Use a function constructor so vitest class detection is satisfied.
    TaskService: function MockTaskService() { return mockTaskService; },
    SqliteTaskStore: vi.fn(),
    resolveDbPath: vi.fn(() => '/tmp/tasks.db'),
...
function sampleTask(overrides: Record<string, unknown> = {}) {
    return {
        taskId: 'task-20260701120000-aaaa',
        title: 'Sample task',
        summary: null,
        feature: 'demo',
        status: 'open',
...
describe('task command', () => {
    ...
    describe('list', () => {
        it('renders a table by default', async () => {
            mockTaskService.list.mockResolvedValue([sampleTask(), sampleTask({ taskId: 'task-20260701120001-bbbb', title: 'Other' })]);
            const program = new Command();
            registerTaskCommand(program);
    ...
    describe('show', () => {
        it('resolves the id and prints JSON with --events', async () => {
            ...
            const events = [{ eventId: 'evt-1', type: 'task.created', ts: 't', actor: null, taskId: task.taskId, payload: {} }];
    ...
    describe('phase / status / progress / next', () => {
        it('sets phase by feature key', async () => {
            ...
            expect(mockTaskService.setPhase).toHaveBeenCalledWith('task-20260701120000-aaaa', 'design', expect.any(Object));
        });
    ...
    describe('blocker', () => {
        it('adds a blocker with variadic text', async () => {
            const task = sampleTask();
            mockTaskService.resolveTask.mockResolvedValue(task);
            mockTaskService.addBlocker.mockResolvedValue({ task, blockerId: 'blk-1' });
        ...
        it('resolves a blocker by id', async () => {
            ...
            const program = new Command();
            registerTaskCommand(program);
            await program.parseAsync(['node', 'test', 'task', 'blocker', 'demo', 'resolve', 'blk-1', '--json']);
            expect(mockTaskService.resolveBlocker).toHaveBeenCalledWith(task.taskId, 'blk-1', expect.any(Object));
        });
    });
    ...
    describe('evidence', () => {
        it('records passing evidence with command and exit code', async () => {
            const task = sampleTask();
            mockTaskService.resolveTask.mockResolvedValue(task);
            mockTaskService.addEvidence.mockResolvedValue({ task, evidenceId: 'evd-1' });
    ...
    describe('event', () => {
        it('appends a custom event defaulting to task.custom with parsed payload', async () => {
            const task = sampleTask();
            mockTaskService.resolveTask.mockResolvedValue(task);
            const event = { eventId: 'evt-1', taskId: task.taskId, ts: 't', type: 'task.custom', actor: null, payload: { name: 'tick' } };
        ...
        it('reads payload from @file', async () => {
            ...
            mockTaskService.addEvent.mockResolvedValue({
                eventId: 'evt-1', taskId: task.taskId, ts: 't', type: 'task.custom', actor: null, payload: {},
            });
