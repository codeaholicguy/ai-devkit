...
describe("setup command", () => {
  ...
  it("warns with platform-aware instructions and continues setup when tmux is missing", async () => {
    ...
    expect(mockInspectTmux.mock.invocationCallOrder[0]).toBeLessThan(
      mockSetupService.run.mock.invocationCallOrder[0],
    );
    expect(mockUi.table.mock.invocationCallOrder[0]).toBeLessThan(
      mockUi.text.mock.invocationCallOrder[0],
    );
    expect(mockUi.table.mock.invocationCallOrder[0]).toBeLessThan(
      mockUi.warning.mock.invocationCallOrder[0],
    );
  ...
  it("fails for unsupported agents before running setup", async () => {
    ...
    expect(mockUi.error).toHaveBeenCalledWith(
      "Unsupported setup agent: cursor. Supported agents: codex, pi.",
    );
