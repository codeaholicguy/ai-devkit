...
vi.mock("../../lib/EnvironmentSelector.js", () => ({
  EnvironmentSelector: vi.fn(function () {
    return mockEnvironmentSelector;
  }),
}));

vi.mock("../../lib/PhaseSelector.js", () => ({
  PhaseSelector: vi.fn(function () {
    return mockPhaseSelector;
  }),
}));
...
describe("init command", () => {
  ...
  describe("template mode", () => {
    ...
    it("sets non-zero exit code when template loading fails", async () => {
      mockLoadInitTemplate.mockRejectedValue(
        new Error("Invalid template at /tmp/init.yaml: bad field"),
      );
