...
const {
  ...
  class AgentNameInUseError extends Error {
    constructor(
  ...
  class AgentPidPollTimeoutError extends Error {
    constructor(
      public agentName: string,
  ...
  class AgentRuntimeUnavailableError extends Error {
    constructor(
  ...
  class AgentTerminalNotFoundError extends Error {
    constructor(
      public agentName: string,
      public pid: number,
    ) {
...
vi.mock(
  ...
  () => ({
    ...
    ClaudePrintAgentService: vi.fn(function () {
      return mockDurableService;
    }),
    CodexPrintAgentService: vi.fn(function () {
      return mockCodexPrintService;
    }),
    PiPrintAgentService: vi.fn(function () {
      return mockPiPrintService;
    }),
    ...
    focusAgent: (...args: any[]) => mockFocusAgent(...args),
    sendAgentPrompt: (...args: any[]) => mockSendAgentPrompt(...args),
    TmuxUnavailableError,
    AgentNameInUseError,
    AgentPidPollTimeoutError,
    AgentRuntimeUnavailableError,
...
describe("agent command", () => {
  ...
  beforeEach(() => {
    ...
    mockStartAgent.mockReset().mockImplementation(async (opts: any) => {
      if (opts.runtimeProvider === "herdr") {
        ...
        const result = await mockHerdrStartAgent({
          name: opts.name,
          cwd: opts.cwd,
          kind: opts.type === "gemini_cli" ? "gemini" : opts.type,
          args: [],
      ...
      if (!(await mockTmuxIsAvailable())) throw new TmuxUnavailableError();
      return {
        name: opts.name,
        type: opts.type,
        pid: 12345,
    ...
    mockSendAgentPrompt
      ...
      .mockImplementation(async (agent: any, prompt: string, deps: any) => {
        const registryEntry = deps.registry?.lookup(agent.name);
        if (registryEntry?.runtime === "herdr") {
          await mockHerdrSend({ runtimeRef: registryEntry.runtimeRef, prompt });
          return;
        }
        ...
        await deps.writer(location, prompt);
      });
...
Waiting on user input`,
  ...
  it("reports when an agent terminal cannot be found", async () => {
    const agent = {
      name: "repo-a",
      status: AgentStatus.WAITING,
      summary: "A",
    ...
    expect(mockFocusManager.findTerminal).toHaveBeenCalledWith(10);
    expect(mockFocusManager.focusTerminal).not.toHaveBeenCalled();
    expect(mockSpinner.fail).toHaveBeenCalledWith(
      'Could not find terminal window for agent "repo-a" (PID: 10).',
    );
  });

  it("reports when terminal focus fails after locating the agent", async () => {
    const agent = {
      name: "repo-a",
      status: AgentStatus.WAITING,
      summary: "A",
  ...
  it("kills a resolved agent and reports tmux cleanup", async () => {
    ...
    expect(ui.success).toHaveBeenCalledWith(
      'Stopped agent "repo-a" (PID 10) and tmux session "repo-a".',
    );
  });
  ...
  it("starts a durable Claude durable agent without tmux", async () => {
    ...
    expect(mockDurableService.create).toHaveBeenCalledWith({
      name: "reviewer",
      cwd: process.cwd(),
    });
    expect(ui.success).toHaveBeenCalledWith(
  ...
  it("rejects the old print mode name for durable agent start", async () => {
    ...
    expect(ui.error).toHaveBeenCalledWith(
      'Failed to start agent: Unsupported agent mode "print". Supported: interactive, durable.',
    );
    expect(process.exit).toHaveBeenCalledWith(1);
  ...
  it("starts a durable Codex print agent unbound without tmux", async () => {
    ...
    expect(mockCodexPrintService.create).toHaveBeenCalledWith({
      name: "reviewer",
      cwd: process.cwd(),
    });
  ...
  it("starts a durable Pi agent without tmux", async () => {
    ...
    expect(mockPiPrintService.create).toHaveBeenCalledWith({
      name: "reviewer",
      cwd: process.cwd(),
    });
  ...
  it("dispatches durable send to the persisted Pi provider", async () => {
    ...
    mockDurableRepository.resolve.mockResolvedValue(durableAgent);
    mockPiPrintService.send.mockResolvedValue({
      agentId: durableAgent.id,
      agentName: durableAgent.name,
      result: "done",
      exitCode: 0,
  ...
  it("sends synchronously to an exact durable-agent id without terminal injection", async () => {
    ...
    mockDurableRepository.resolve.mockResolvedValue(durableAgent);
    mockDurableService.send.mockResolvedValue({
      ...durableAgent,
      result: "\x1b]0;unsafe\x07review complete",
      exitCode: 0,
    });
  ...
  it("fails when both a message argument and --stdin are provided", async () => {
    ...
    expect(ui.error).toHaveBeenCalledWith(
      "Failed to send message: Use either a message argument or --stdin, not both.",
    );
    expect(process.exit).toHaveBeenCalledWith(1);
    expect(mockTtyWriterSend).not.toHaveBeenCalled();
  });
  ...
  it("fails before sending when no send target selector is provided", async () => {
    ...
    expect(ui.error).toHaveBeenCalledWith(
      "Failed to send message: Use exactly one of --id or --group.",
    );
    expect(process.exit).toHaveBeenCalledWith(1);
  ...
  it("rejects --wait with --group before sending", async () => {
    ...
    expect(ui.error).toHaveBeenCalledWith(
      "Failed to send message: Use --wait only with --id; group wait mode is not supported.",
    );
    expect(process.exit).toHaveBeenCalledWith(1);
    expect(mockTtyWriterSend).not.toHaveBeenCalled();
  });
  ...
  it("rejects --json with --group before sending", async () => {
    ...
    expect(ui.error).toHaveBeenCalledWith(
      "Failed to send message: Use --json only with --id --wait; group JSON output is not supported.",
    );
    expect(process.exit).toHaveBeenCalledWith(1);
    expect(mockTtyWriterSend).not.toHaveBeenCalled();
  });
  ...
  it("uses the provided timeout duration for send --wait", async () => {
    ...
    mockAgentAdapter.getConversation
      .mockReturnValueOnce([])
      .mockReturnValue([{ role: "assistant", content: "done" }]);
    mockFocusManager.findTerminal.mockResolvedValue(location);
    mockTtyWriterSend.mockResolvedValue(undefined);
  ...
  it("fails before sending when --timeout is used without --wait", async () => {
    ...
    expect(ui.error).toHaveBeenCalledWith(
      "Failed to send message: Use --timeout only with --wait.",
    );
    expect(process.exit).toHaveBeenCalledWith(1);
    expect(mockManager.listAgents).not.toHaveBeenCalled();
    expect(mockTtyWriterSend).not.toHaveBeenCalled();
  ...
  it("fails before sending when --timeout is invalid", async () => {
    ...
    expect(ui.error).toHaveBeenCalledWith(
      "Failed to send message: Invalid --timeout. Expected positive integer milliseconds. Example: 30000.",
    );
    expect(process.exit).toHaveBeenCalledWith(1);
    expect(mockManager.listAgents).not.toHaveBeenCalled();
    expect(mockTtyWriterSend).not.toHaveBeenCalled();
  ...
  it("fails and does not send when --wait target has no session file", async () => {
    ...
    expect(ui.error).toHaveBeenCalledWith(
      'Failed to send message: No session file found for agent "repo-a"; cannot wait for response.',
    );
    expect(process.exit).toHaveBeenCalledWith(1);
    expect(mockTtyWriterSend).not.toHaveBeenCalled();
  });
  ...
  it("fails when --wait terminal cannot be found", async () => {
    ...
    expect(ui.error).toHaveBeenCalledWith(
      'Failed to send message: Cannot find terminal for agent "repo-a" (PID: 10).',
    );
    expect(process.exit).toHaveBeenCalledWith(1);
    expect(mockTtyWriterSend).not.toHaveBeenCalled();
  });
  ...
  it("writes busy-agent warning to stderr in --wait mode", async () => {
    ...
    mockManager.listAgents
      ...
      .mockResolvedValue([{ ...agent, status: AgentStatus.WAITING }]);
    mockManager.resolveAgent.mockReturnValue(agent);
    mockManager.getAdapter.mockReturnValue(mockAgentAdapter);
    mockAgentAdapter.getConversation
      .mockReturnValueOnce([])
  ...
  describe("agent rename", () => {
    ...
    it("shows error and exits 1 when agent is not found", async () => {
      mockRegistry.rename.mockImplementation(() => {
        throw new RenameNotFoundError("old-name");
      });
    ...
    it("shows error and exits 1 when new name is already in use", async () => {
      mockRegistry.rename.mockImplementation(() => {
        throw new RenameConflictError("new-name");
      });
