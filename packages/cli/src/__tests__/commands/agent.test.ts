...
Waiting on user input`,
  ...
  it('kills a skipped empty-session start row by its registry name', async () => {
    ...
    mockManager.listAgents.mockResolvedValue([]);
    mockRegistry.lookup.mockReturnValue(retained);
    mockKillAgent.mockResolvedValue({ agentName: 'old-session', pid: 123, tmuxSession: 'old-session' });

    const program = new Command();
    registerAgentCommand(program);
    await program.parseAsync(['node', 'test', 'agent', 'kill', 'old-session']);

    expect(mockKillAgent).toHaveBeenCalledWith(retained, expect.objectContaining({ registry: mockRegistry }));
    expect(ui.success).toHaveBeenCalledWith(
      'Stopped agent "old-session" (PID 123) and tmux session "old-session".',
    );
  });
