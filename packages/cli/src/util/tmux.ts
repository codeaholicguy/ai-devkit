...
export async function resolveTmuxInstallInstructions(
  ...
  if (deps.platform === "win32") {
    return {
      command: null,
      message:
        "Native Windows managed interactive sessions are unsupported. Use a supported WSL distribution and install tmux inside it.",
    };
  }
  if (deps.platform === "freebsd" || deps.platform === "openbsd") {
    return {
      command: null,
      message:
        "BSD managed interactive sessions are not currently supported; consult your system tmux documentation.",
    };
  }
  ...
  if (distroKeys.includes("nixos")) {
    return {
      ...
      message:
        "Nix environments are user-managed; use a declarative configuration or run: nix shell nixpkgs#tmux.",
    };
  }
