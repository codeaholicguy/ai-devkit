...
import { renderMarkdownRows, type MarkdownPreviewSpan } from "./render/markdownPreview.js";
...
export function buildPreviewViewportFromRows(
  ...
  const continuation =
    hasAbove && firstVisible?.kind === "content" && firstVisible.role
      ? ` · ${firstVisible.role} continued`
      : "";
  const indicator: PreviewViewportRow[] =
    hasAbove || hasBelow
      ? [
          {
            kind: "indicator",
...
export function buildPreviewViewport(
  ...
  return buildPreviewViewportFromRows(
    buildPreviewRows(messages, contentWidth),
    maxLines,
    requestedOffset,
  );
}
...
export function getPreviewChannelStatusText(
  channelStatus: AgentChannelStatus | undefined,
): string | null {
  return channelStatus ? `Connected: ${channelStatus.channelName}` : null;
}

const MetadataHeader: React.FC<{ agent: AgentInfo; channelStatus?: AgentChannelStatus }> = ({
  agent,
  channelStatus,
}) => (
  <Box>
...
const PreviewPaneInner: React.FC<PreviewPaneProps> = ({
  ...
  const viewport =
    rows.length > 0 ? buildPreviewViewportFromRows(rows, bodyMaxLines, adjustedScrollOffset) : null;
      ...
      <>
        {viewport?.rows.map((row, idx) =>
          row.kind === "indicator" ? (
            ...
            <Box key={idx}>
              {row.timestamp ? (
                <Text dimColor>[{new Date(row.timestamp).toLocaleTimeString()}] </Text>
              ) : null}
              <Text color={ROLE_COLOR[row.role]} bold>
                {row.role}:
              </Text>
            </Box>
          ) : row.kind === "separator" ? (
