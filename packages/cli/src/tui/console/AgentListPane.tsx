...
export function getHighlightedNameSegments(
  name: string,
  query: string,
  width: number,
): HighlightedNameSegment[] {
  const clipped = clip(name, width);
  const visibleName = clipped.endsWith("…") ? clipped.slice(0, -1) : clipped;
  const positions =
    findMatchPositions(name, query)?.flatMap((position, index, all) => {
      if (index % 2 === 1) return [];
  ...
  if (cursor < visibleName.length)
    segments.push({ text: visibleName.slice(cursor), matched: false });
  if (clipped.endsWith("…")) segments.push({ text: "…", matched: false });
  return segments;
}

export function clampAgentListScrollOffset(
  offset: number,
  length: number,
  maxVisible: number,
): number {
    ...
    <Box flexDirection="column" width={innerWidth}>
      <Box flexDirection="row" width={innerWidth}>
        ...
        <Box width={nameW} flexShrink={0} overflow="hidden">
          <Text color={accent} bold={isSelected}>
            {getHighlightedNameSegments(agent.name, filterText, nameW).map((segment, index) => (
              <Text key={index} bold={isSelected || segment.matched}>
                {segment.text}
              </Text>
            ))}
          </Text>
        ...
        <Box width={CHANNEL_W} flexShrink={0}>
          <Text color={channelStatus ? TUI_COLORS.success : undefined}>
            {getAgentChannelMarker(channelStatus)}
          </Text>
        </Box>
      </Box>
      <Box flexDirection="row" width={innerWidth}>
  ...
  <Panel width="100%" height={3} focused={editing} paddingX={1}>
    {editing ? (
      <>
        ...
        <TextInput value={filterText} onChange={onChange} onSubmit={onSubmit} placeholder="name" />
      </>
    ) : filterText ? (
      <>
  ...
  return (
    <Box flexDirection="column" width={innerWidth}>
      <Box width={innerWidth}>
        ...
        <Text dimColor>
          ({filterInPlay ? `${orderedAgents.length}/${totalAgents}` : totalAgents})
        </Text>
      ...
      {error && totalAgents === 0 ? (
        ...
        <Text dimColor>
          {filterInPlay ? `No agents match "${filterText}"` : "No running agents."}
        </Text>
      ) : (
        visibleAgents.map((agent, i) => (
          <React.Fragment key={agent.name}>
            {i > 0 && (
              <Box width={innerWidth}>
                <Text dimColor>
                  {getAgentDivider(innerWidth, isPinnedBoundary(orderedAgents, scrollOffset + i))}
                </Text>
              </Box>
            )}
            <AgentRow
