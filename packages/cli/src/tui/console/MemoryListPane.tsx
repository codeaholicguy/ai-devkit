...
export function getMemoryListStatusText(input: {
  ...
  if (input.displayState === "error")
    return truncate(input.error ?? "Could not load memory.", input.width);
...
export function getVisibleMemoryItems(
    ...
    <Panel width={width} height={height} focused paddingX={1} flexDirection="column" flexShrink={0}>
      ...
      <Box flexDirection="column" marginTop={1}>
        {displayState === "loading" ? (
          ...
          rows.map((row, index) => (
            <Box
              key={`${visibleItems[index]?.id ?? index}`}
