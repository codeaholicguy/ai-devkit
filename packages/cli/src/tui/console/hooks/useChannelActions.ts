...
export function getChannelActionError(
  actionName: "channel start" | "channel stop",
  result: ActionResult,
): string | null {
  if (result.error) return result.error;
  if (result.exitCode !== 0 && result.exitCode !== null)
    return `${actionName} exited ${result.exitCode}`;
  return null;
}
...
export function useChannelActions({
  ...
  const openChannelSelect = useCallback(
    (agent: AgentInfo | null) => {
      if (!agent) return;
  ...
  const startChannel = useCallback(
    (channelName: string, agentName: string) => {
  ...
  const stopAgentChannel = useCallback(
    (agent: AgentInfo | null) => {
