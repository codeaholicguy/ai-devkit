...
function isRowPrefix(span: MarkdownPreviewSpan | undefined): boolean {
  return Boolean(
    span &&
    (span.text === "  " || span.text === "│ " || span.text === "• " || /^\d+\. $/u.test(span.text)),
  );
}
...
export function wrapMarkdownRow(
  ...
  for (const span of content) {
    for (const piece of span.text.match(/\s+|\S+/gu) ?? [""]) {
      ...
      while (remaining) {
        const firstGrapheme =
          graphemeSegmenter.segment(remaining)[Symbol.iterator]().next().value?.segment ?? "";
        if (
...
function renderBlock(token: Token): MarkdownPreviewRow[] {
  switch (token.type) {
    case "heading":
      return [
        applyStyle(renderInline((token as Tokens.Heading).tokens), {
          bold: true,
    ...
    case "code": {
      ...
      return [
        ...
        ...code.text
          .split("\n")
          .map((line) => [{ text: "  " }, { text: line, color: TUI_COLORS.warning }]),
        [
          {
    ...
    case "blockquote":
      return (token as Tokens.Blockquote).tokens
        .flatMap(renderBlock)
        .map((row) => [{ text: "│ ", dimColor: true }, ...row]);
    case "text": {
      const text = token as Tokens.Text;
      return [text.tokens ? renderInline(text.tokens) : [{ text: text.text }]];
    }
