...
export async function installMcpServers(
  ...
  const selectedGenerators = GENERATORS.filter(
    (g) => environments.includes(g.agentType) && hasMcpSupport(g.agentType),
  );
  ...
  for (const generator of activeGenerators) {
    ...
    try {
      ...
      if (plan.conflictServers.length > 0) {
        ...
        plan.resolvedConflicts =
          typeof resolved === "string"
            ? resolved === "overwrite"
              ? [...plan.conflictServers]
              : []
            : await resolved;
        report.conflicts += plan.conflictServers.length - plan.resolvedConflicts.length;
        const unresolved = plan.conflictServers.filter(
          (name) => !plan.resolvedConflicts.includes(name),
        );
        report.items.push(
          ...unresolved.map((name) => ({
            name,
      ...
      if (toInstall > 0) {
        ...
        report.items.push(
          ...[...plan.newServers, ...plan.resolvedConflicts].map((name) => ({
            name,
            target,
