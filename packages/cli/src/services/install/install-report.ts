...
export function renderApplicationReport(report: InstallReport, title = "Install Summary"): void {
  ...
  const details = report.items.filter(
    (item) => (item.status === "failed" || item.status === "conflict") && item.message,
  );
