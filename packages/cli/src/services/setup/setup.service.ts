...
export function createSetupService(deps: SetupServiceDeps = {}): SetupService {
  ...
  return {
    async run(options: SetupRunOptions = {}): Promise<SetupReport> {
      ...
      const selectedDefinitions = setupDefinitions.filter((definition) =>
        agents.includes(definition.agent),
      );
      ...
      for (const definition of selectedDefinitions) {
        if (!(await fs.pathExists(join(context.homeDir, definition.dotFolder)))) {
          results.push(
            skipped(definition.agent, "setup", `~/${definition.dotFolder} does not exist.`),
          );
          continue;
        }
...
async function setupCodexSessionHook(context: SetupStepContext): Promise<SetupStepResult> {
  ...
  const sourceHooks = (await fs.readJson(
    join(context.assetRoot, "codex", "hooks.json"),
  )) as CodexHooksJson;
  ...
  const alreadyConfigured = sessionStart.some(
    (entry) =>
...
async function setupClaudePromptHook(context: SetupStepContext): Promise<SetupStepResult> {
  ...
  const alreadyConfigured = existingEntries.some(
    (entry) =>
      Array.isArray(entry?.hooks) &&
...
function getSessionStartEntries(hooksJson: CodexHooksJson): CodexSessionStartEntry[] {
  ...
  return Array.isArray(sessionStart) ? sessionStart.filter(isSessionStartEntry) : [];
}
