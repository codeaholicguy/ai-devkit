...
async function projectConfigCheck(
  rt: Runtime,
): Promise<{ check: ProjectConfigCheck; raw: Record<string, unknown> | null }> {
  ...
  if (!parsed)
    return {
      ...
      check: {
        ...
        environments: [],
        status: "fail",
        errors: ["project configuration is invalid JSON or not an object"],
      },
    };
  ...
  const valid =
    ...
    environments.length === parsed.environments.length &&
...
async function versionCheck(rt: Runtime): Promise<VersionCheck> {
  try {
    ...
    return {
      installedVersion: rt.installedVersion,
      latestVersion,
      updateAvailable,
      latestVersionSource: "npm",
      status: "pass",
    ...
    return {
      ...
      latestVersionSource: "npm",
      status: "warn",
      errors: ["latest npm version is unavailable"],
    };
  }
}
...
async function tmuxCheck(rt: Runtime): Promise<TmuxCheck> {
  ...
  if (inspection.state === "missing") {
    return {
      ...
      version: null,
      status: "fail",
      errors: ["tmux was not found on PATH"],
    };
  }
  return {
    path: "tmux",
    ...
    version: null,
    status: "fail",
    errors: ["tmux version probe failed"],
  };
}
...
function channelConnection(name: string, value: unknown): ChannelConnection {
  ...
  if (type === "telegram") {
    ...
    schemaValid =
      schemaValid &&
      credentialsPresent &&
      nonEmpty(config?.botUserId) &&
      nonEmpty(config?.workspaceId) &&
      config?.transport === "socket-mode" &&
      config?.audience === "dm";
  } else {
    schemaValid = false;
  }
...
async function channelsCheck(rt: Runtime): Promise<ChannelsCheck> {
  ...
  if (!parsed || !channelRecord) {
    const config: ChannelConfigCheck = {
      ...
      validSchema: false,
      errors: ["channel configuration is invalid"],
    };
    return { config, connections: [], readyCount: 0 };
  }
  const connections = Object.entries(channelRecord).map(([name, entry]) =>
    channelConnection(name, entry),
  );
  ...
  return {
    config,
    connections,
    readyCount: connections.filter((item) => item.ready).length,
  };
}
...
export async function getStatusReport(options: StatusServiceOptions = {}): Promise<StatusReport> {
  ...
  const partial = {
    generatedAt: rt.now().toISOString(),
    aiDevkit,
    project: { cwd: rt.cwd, config: project.check },
    agents,
    tmux,
    registries,
    channels,
