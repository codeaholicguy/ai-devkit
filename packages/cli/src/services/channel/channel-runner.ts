...
export function setupInputHandler(
  ...
  channel.onMessage(async (msg) => {
    ...
    if (msg.chatId !== chatIdRef.value) {
      ...
      debug(
        `Rejected message from ${onAuthorize ? "unauthorized" : "inactive"} chat ID: ${msg.chatId}`,
      );
...
function formatPromptMessage(toolName: string, toolInput: Record<string, unknown>): string {
  if (toolName === "AskUserQuestion") {
    const question =
      typeof toolInput.question === "string" ? toolInput.question : JSON.stringify(toolInput);
    ...
    if (agent.sessionFilePath) {
      ...
      try {
        ...
        if (conversation.length !== lastReportedLength) {
          debug(
            `Conversation length changed: ${lastReportedLength} -> ${conversation.length} (lastMessageCount=${lastMessageCount}, new=${newMessages.length})`,
          );
        ...
        try {
          ...
          if (!handled) {
            await channel.sendMessage(
              chatIdRef.value,
              formatPromptMessage(agentRequest.toolName, agentRequest.toolInput),
            );
          }
...
export async function runChannelBridge(input: RunChannelBridgeInput): Promise<void> {
  ...
  if (channelEntry.type === TELEGRAM_CHANNEL_TYPE) {
    ...
    channel = telegram;
    chatIdRef.value =
      telegramConfig.authorizedChatId !== undefined
    ...
    const telegramQuestions = new AskUserQuestionService(telegram, (key) =>
      TtyWriter.sendKey(terminalLocation, key),
    );
    ...
    const slackQuestions = new SlackQuestionService(slack, (key) =>
      TtyWriter.sendKey(terminalLocation, key),
    );
  ...
  ui.success(
    `Bridge started: ${input.channelName} (${channelEntry.type}) <-> Agent "${agent.name}" (PID: ${agent.pid})`,
  );
