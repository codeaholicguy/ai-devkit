  ...
  for (const command of commands) {
    if (seenCommands.has(command.name)) {
      throw new ValidationError(
        `Plugin ${pluginName} declares duplicate command "${command.name}".`,
      );
    }
    ...
    if (builtInCommandNames.has(command.name)) {
      throw new ValidationError(
        `Plugin ${pluginName} command "${command.name}" conflicts with a built-in command.`,
      );
    }

    if (!JAVASCRIPT_ENTRY_EXTENSIONS.has(path.extname(command.entry))) {
      throw new ValidationError(
        `Plugin ${pluginName} command "${command.name}" must point to a JavaScript entrypoint.`,
      );
    }
  }
...
function normalizeCommand(
  ...
  const description =
    typeof rawCommand.description === "string" ? rawCommand.description.trim() : undefined;
  ...
  if (!COMMAND_NAME_PATTERN.test(name)) {
    throw new ValidationError(
      `Plugin ${pluginName} command "${name}" must use lowercase letters, numbers, and hyphens only, and start with a letter.`,
    );
  }
