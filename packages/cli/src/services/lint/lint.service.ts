...
export function runLintChecks(
  ...
  const providedDependencies = Array.isArray(phasesOrDependencies)
    ? dependencies
    : phasesOrDependencies;
...
function runFeatureChecks(
  ...
  checks.push(
    ...runFeatureDocsRules(cwd, docsDir, phases, featureValidation.target.normalizedName, deps),
  );
