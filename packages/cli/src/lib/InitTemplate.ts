  ...
  if (candidate.skills !== undefined) {
    ...
    result.skills = candidate.skills.map((entry, index) => {
      ...
      if (typeof registry !== "string" || registry.trim().length === 0) {
        throw validationError(
          resolvedPath,
          `"skills[${index}].registry" must be a non-empty string`,
        );
      }
...
function validateMcpServers(
  ...
  for (const [name, value] of Object.entries(servers)) {
    ...
    if (
      ...
      !MCP_TRANSPORTS.includes(server.transport as McpTransport)
    ) {
      throw validationError(
        resolvedPath,
        `"${prefix}.transport" must be one of: ${MCP_TRANSPORTS.join(", ")}`,
      );
    }
    ...
    if (transport === "stdio") {
      ...
      if (server.args !== undefined) {
        if (
          ...
          !server.args.every((a: unknown) => typeof a === "string")
        ) {
      ...
      if (typeof server.url !== "string" || server.url.trim().length === 0) {
        throw validationError(
          resolvedPath,
          `"${prefix}.url" is required for ${transport} transport`,
        );
      }
...
export async function loadInitTemplate(templatePath: string): Promise<InitTemplateConfig> {
  ...
  if (template.registries) {
    template.registries = await normalizeRegistrySources(
      template.registries,
