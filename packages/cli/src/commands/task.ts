...
import type { Actor, Task, TaskStatus } from '@ai-devkit/task-manager';
...
/**
 * Shared attribution flags parsed from commander options.
 */
...
function createStore(storeFlag?: string): TaskService {
    return new TaskService(new SqliteTaskStore(resolveDbPath(storeFlag)));
}

/** Output JSON or a human value. */
...
/**
 * Resolve a task reference to a Task, printing an error on ambiguity/missing.
 */
...
export function registerTaskCommand(program: Command): void {
    ...
    const addAttributionFlags = (cmd: Command): Command =>
        cmd
            .option('--store <db-path>', 'Override the tasks database path (env: AIDEVKIT_TASKS_DB)')
    ...
    addAttributionFlags(
        ...
        withErrorHandler('create task', async (opts) => {
            const service = createStore(opts.store);
