...
import type { GlobalInstalledSkill, InstalledSkill, SkillInstallResult, SkillRemoveResult } from '../../services/skill/skill.types.js';
import type { UpdateSummary } from '../../services/skill/registry/skill-registry.service.js';
import type { SkillEntry, SkillIndexRebuildResult } from '../../services/skill/index/skill-index.service.js';

export function renderSkillInstallResult(result: SkillInstallResult): void {
  for (const item of result.items) {
    const suffix = item.action === 'skipped'
      ? 'already exists, skipped'
      : item.action;
    ui.text(`  -> ${item.target} (${suffix})`);
  }

  ui.text(`Successfully installed: ${[...new Set(result.items.map(item => item.skillName))].join(', ')}`);
...
export function renderSkillRemoveResult(result: SkillRemoveResult): void {
  ...
  if (result.removedTargets.length === 0) {
    ui.warning(result.scope === 'global'
  ...
  ui.success(result.scope === 'global'
