...
export function registerChannelCommand(program: Command): void {
  ...
  channelCommand
    ...
    .action(
      withErrorHandler(
        "connect channel",
        async (type: string, options: { name?: string; debug?: boolean }) => {
          if (options.debug) {
            enableDebug();
          }
          if (type !== TELEGRAM_CHANNEL_TYPE && type !== SLACK_CHANNEL_TYPE) {
            ui.error(
              `Unsupported channel type: ${type}. Supported: ${TELEGRAM_CHANNEL_TYPE}, ${SLACK_CHANNEL_TYPE}`,
            );
            return;
          }
  ...
  channelCommand
    ...
    .action(
      withErrorHandler("list channels", async () => {
        const configStore = new ChannelConfigRepository();
        const config = await configStore.getConfig();
        ...
        const rows = channels.map(([name, entry]) => {
          const identity =
            entry.type === SLACK_CHANNEL_TYPE
              ? ((entry.config as SlackConfig).workspaceName ??
                (entry.config as SlackConfig).workspaceId)
              : `@${(entry.config as TelegramConfig).botUsername}`;
          const authorization =
            entry.type === SLACK_CHANNEL_TYPE
              ? "n/a"
              : (entry.config as TelegramConfig).authorizedChatId
                ? "yes"
                : "no";
          return [
            name,
  ...
  channelCommand
    ...
    .action(
      withErrorHandler("disconnect channel", async (name: string) => {
    ...
    .action(
      withErrorHandler("channel status", async (name: string | undefined) => {
        ...
        if (channels.length === 0) {
          ui.info(
            channelFilter
              ? `No channel configured with name "${channelFilter}".`
