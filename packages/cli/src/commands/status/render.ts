  ...
  ui.table({
    ...
    rows: [
      [
        ...
        `${report.checks.passed} ready · ${report.checks.warnings} not ready · ${report.checks.failed} fail`,
      ],
      [
        "ai-devkit",
      ...
      [
        ...
        report.tmux.available
          ? `${report.tmux.path} · ${report.tmux.version ?? "unknown"}`
          : "unavailable",
      ],
      [
        "registries",
  ...
  for (const [agent, item] of agentEntries(report)) {
    details.push(
      [
        ...
        item.executable.path ?? "not found",
      ],
