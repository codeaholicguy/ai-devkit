...
export function registerAgentGroupCommand(agentCommand: Command): void {
  const groupCommand = agentCommand.command("group").description("Manage named groups of agents");
  ...
  groupCommand
    ...
    .option(
      "--agent <identifier>",
      "Agent identifier to include in the group",
      collectAgentMember,
      [] as string[],
    )
    .action(
      withErrorHandler(
        "manage agent group",
        async (name: string, options: { agent?: string[] }) => {
  ...
  groupCommand
    ...
    .action(
      withErrorHandler("manage agent group", async (name: string, identifier: string) => {
  ...
  groupCommand
    ...
    .action(
      withErrorHandler("manage agent group", async (name: string) => {
  ...
  groupCommand
    ...
    .action(
      withErrorHandler("manage agent group", async () => {
        ...
        ui.table({
          ...
          columnStyles: [(text) => chalk.cyan(text), (text) => text],
        });
      }),
    );
