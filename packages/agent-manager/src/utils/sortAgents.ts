...
function compareByLastActive(a: AgentInfo, b: AgentInfo): number {
  const ta =
    a.lastActive instanceof Date ? a.lastActive.getTime() : new Date(a.lastActive).getTime();
  const tb =
    b.lastActive instanceof Date ? b.lastActive.getTime() : new Date(b.lastActive).getTime();
