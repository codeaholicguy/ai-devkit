...
type ProviderAgent<Provider extends DurableProvider> = Extract<
  DurableAgent,
  { provider: Provider }
>;
...
export async function runDurableAgent<
  Provider extends DurableProvider,
  Result extends DurableRunResult,
>(
  ...
  await options.repository.completeRun(
    target.id,
    acquired.token,
    createSucceededCompletion(result),
  );
