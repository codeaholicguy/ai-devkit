...
export class HerdrAgentRuntime implements InteractiveAgentRuntime {
  ...
  async startAgent(input: RuntimeStartInput): Promise<RuntimeStartResult> {
    const workspace = extractWorkspaceStart(
      await this.client.runJson(["workspace", "create", "--cwd", input.cwd, "--label", input.name]),
    );
  ...
  private async runAgentStartJson(args: string[]): Promise<unknown> {
    ...
    throw (
      lastBusyError ??
      new HerdrRuntimeError("Herdr agent start failed because the target pane stayed busy.", {
