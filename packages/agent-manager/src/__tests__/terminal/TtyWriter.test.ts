...
describe("TtyWriter", () => {
  ...
  describe("tmux", () => {
    ...
    it("throws on tmux failure", async () => {
      ...
      await expect(TtyWriter.send(location, "hello")).rejects.toThrow("tmux not running");
    });
  });
  ...
  describe("iTerm2", () => {
    ...
    it("throws when session not found", async () => {
      ...
      await expect(TtyWriter.send(location, "test")).rejects.toThrow("iTerm2 session not found");
    });
    ...
    it("throws when session disappears before Enter", async () => {
      ...
      await expect(TtyWriter.send(location, "test")).rejects.toThrow(
        "iTerm2 session disappeared before Enter",
      );
    });
  });
  ...
  describe("Terminal.app", () => {
    ...
    it("throws when tab not found", async () => {
      ...
      await expect(TtyWriter.send(location, "test")).rejects.toThrow("Terminal.app tab not found");
    });
    ...
    it("throws when tab disappears before Enter", async () => {
      ...
      await expect(TtyWriter.send(location, "test")).rejects.toThrow(
        "Terminal.app tab disappeared before Enter",
      );
    });
  });
  ...
  describe("WezTerm", () => {
    ...
    it("keeps the whole message out of argv and writes it verbatim to stdin", async () => {
      ...
      expect(textArgs).toEqual(["cli", "send-text", "--pane-id", "7"]);
  ...
  describe("unsupported terminal", () => {
    it("throws for unknown terminal type", async () => {
      ...
      await expect(TtyWriter.send(location, "test")).rejects.toThrow(
        "Cannot send input: unsupported terminal type",
      );
    });
  });
  ...
  describe("sendKey — unsupported terminal", () => {
    it("throws for unknown terminal type", async () => {
      ...
      await expect(TtyWriter.sendKey(location, "1")).rejects.toThrow(
        "Cannot send key: unsupported terminal type",
      );
    });
  });
});
