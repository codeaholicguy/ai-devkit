...
describe("DurableAgentRepository SQLite concurrency", () => {
  it("allows exactly one acquisition across two connections", async () => {
    ...
    const processInspector = {
      getIdentity: (pid: number) => (pid === process.pid ? identity : null),
    };
  ...
  it("rejects stale tokens and caps the persisted completion summary", async () => {
    ...
    await expect(
      repository.recordProviderProcess(agent.id, "stale", { pid: 42, startedAt: "provider" }),
  ...
  it("does not reconcile over ownership changed after process inspection", async () => {
    ...
    inspect = () => {
      inspect = undefined;
      other
        .prepare(`UPDATE durable_agents SET
    ...
    expect(
      other
        .prepare("SELECT state, active_run_token FROM durable_agents WHERE id = ?")
        .get(agent.id),
  ...
  it("maps a corrupt database to a store error", () => {
    ...
    expect(() => new DurableAgentRepository({ dbPath })).toThrow(
      /Cannot open durable-agent database/,
    );
  });
});
