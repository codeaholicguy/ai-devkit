...
describe("ClaudePrintAgentService", () => {
  it("validates before create and does not run Claude", async () => {
    ...
    const probe = {
      validate: vi.fn().mockResolvedValue({ executable: "claude", version: "2.1.220" }),
    };
    ...
    await expect(
      new Service({ repository, probe, runner }).create({ name: "reviewer", cwd: "/project" }),
  ...
  it("rejects a non-Claude target without acquiring or mutating it", async () => {
    ...
    ).rejects.toMatchObject({ code: "CLAUDE_PRINT_UNSUPPORTED" });
  ...
  it("does not turn a successful completion write failure into a second completion", async () => {
    ...
    const runner = {
      run: vi.fn().mockResolvedValue({ sessionId: "session", result: "answer", exitCode: 0 }),
    };
