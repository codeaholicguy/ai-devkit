...
describe("CodexSessionParser", () => {
  ...
  it("reads current response_item and event_msg messages without mirrored duplicates", () => {
    const filePath = writeJsonl("conversation.jsonl", [
      {
        type: "session_meta",
        payload: { id: "sess-1", cwd: "/repo", timestamp: "2026-03-27T10:00:00Z" },
      },
      {
        type: "response_item",
  ...
  it("maps completed assistant messages as waiting while recent active events stay running", () => {
    const waiting = parser.readSession(
      ...
      [
        JSON.stringify({
          type: "session_meta",
          payload: { id: "waiting", timestamp: new Date().toISOString(), cwd: "/repo" },
        }),
        JSON.stringify({
          type: "event_msg",
    ...
    const running = parser.readSession(
      ...
      [
        JSON.stringify({
          type: "session_meta",
          payload: { id: "running", timestamp: new Date().toISOString(), cwd: "/repo" },
        }),
        JSON.stringify({
          type: "event",
          timestamp: new Date().toISOString(),
          payload: { type: "token_count", message: "Working" },
        }),
      ].join("\n"),
    );
  ...
  it("builds historical summaries from the first non-synthetic user message", () => {
    const filePath = writeJsonl("summary.jsonl", [
      {
        type: "session_meta",
        payload: { id: "summary", cwd: "/repo", timestamp: "2025-01-01T00:00:00Z" },
      },
      {
        type: "response_item",
  ...
  it("returns null or empty results for unreadable and malformed inputs", () => {
    ...
    expect(
      parser.readSession("unused.jsonl", JSON.stringify({ type: "event", payload: {} })),
    ).toBeNull();
