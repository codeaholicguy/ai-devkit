...
import {
  ...
  readPiAssistantText,
  readPiSessionId,
} from "../../../../providers/pi/durable/PiPrintProtocol.js";
...
describe("Pi print pure protocol mapping", () => {
  ...
  it("accepts one expected UUID and rejects invalid, duplicate, and mismatched identities", () => {
    ...
    expect(() => readPiSessionId({}, null, SESSION)).toThrowError(
      expect.objectContaining({ code: "PI_PROTOCOL" }),
    );
    expect(() => readPiSessionId({ id: [SESSION] }, null, SESSION)).toThrowError(
      expect.objectContaining({ code: "PI_PROTOCOL" }),
    );
    expect(() => readPiSessionId({ id: SESSION }, SESSION, SESSION)).toThrowError(
      expect.objectContaining({ code: "PI_PROTOCOL" }),
    );
    expect(() =>
      readPiSessionId({ id: "33333333-3333-4333-8333-333333333333" }, null, SESSION),
  ...
  it("extracts only non-empty completed assistant text", () => {
    ...
    expect(
      readPiAssistantText({ type: "message_end", message: { role: "user", content: "no" } }),
    ).toBeNull();
    expect(
      readPiAssistantText({ type: "message_end", message: { role: "assistant", content: " " } }),
    ).toBeNull();
    expect(
      readPiAssistantText({
        type: "message_end",
    ...
    expect(
      readPiAssistantText({ type: "message_end", message: { role: "assistant", content: null } }),
    ).toBeNull();
    expect(
      readPiAssistantText({
        type: "message_end",
        message: { role: "assistant", content: [null, [], { type: "thinking" }] },
      }),
    ).toBeNull();
    expect(
      readPiAssistantText({
