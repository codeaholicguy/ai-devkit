...
describe("PiStreamParser", () => {
  it("ignores blank lines and normalizes unexpected parser failures", () => {
    ...
    expect(() => parser.result({ code: 0, signal: null })).toThrowError(
      expect.objectContaining({ code: "PI_PROTOCOL", message: "Pi stream processing failed." }),
    );
  });
  ...
  it("includes the terminating signal in process failures", () => {
    ...
    expect(() => parser.result({ code: null, signal: "SIGTERM" })).toThrowError(
      expect.objectContaining({ code: "PI_PROCESS", message: "Pi print run failed (SIGTERM)" }),
    );
  });
});
