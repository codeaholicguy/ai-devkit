...
describe("PiPrintRunner", () => {
  it("binds a first session and returns the last completed assistant text", async () => {
    const fixture = fakeSpawn(events());
  ...
  it("resumes the exact stored session and rejects mismatch", async () => {
    ...
    await expect(
      (await runner(fixture)).run({ agent: agent("healthy"), prompt: "later", onSpawn: vi.fn() }),
  ...
  it.each([
    ["malformed", ["{bad\n"], "PI_PROTOCOL"],
    ["non-object", ["[]\n"], "PI_PROTOCOL"],
    ["truncated", ["{}"], "PI_PROTOCOL"],
    ["missing session", [JSON.stringify({ type: "agent_end" }), ""], "PI_PROTOCOL"],
    [
      "missing result",
      [JSON.stringify({ type: "session", id: SESSION }), JSON.stringify({ type: "agent_end" }), ""],
      "PI_RESULT_MISSING",
    ],
    [
      "missing end",
    ...
    await expect(
      (await runner(fakeSpawn(lines as string[]))).run({
        agent: agent(),
        prompt: "x",
        onSpawn: vi.fn(),
      }),
  ...
  it("rejects oversized output, failed processes, and unverifiable identities", async () => {
    await expect(
      (await runner(fakeSpawn(["x".repeat(20)]), 10)).run({
        agent: agent(),
        prompt: "x",
        onSpawn: vi.fn(),
      }),
  ...
  it("rejects invalid and duplicate session identities", async () => {
    for (const lines of [
      ...
      await expect(
        (await runner(fakeSpawn(lines))).run({
          agent: agent(),
          prompt: "x",
          onSpawn: vi.fn(),
          onSession: vi.fn(),
  ...
  it("kills on spawn persistence failure", async () => {
    ...
    const failure = new Error("cannot persist");
    await expect(
      (await runner(spawnFailure)).run({
        agent: agent(),
        prompt: "x",
        onSpawn: vi.fn().mockRejectedValue(failure),
        onSession: vi.fn(),
      }),
  ...
  it("classifies spawn errors and a missing PID as process failures", async () => {
    ...
    errored.child.stdin = new Writable({
      write(_chunk, _encoding, callback) {
        callback();
      },
      final(callback) {
        queueMicrotask(() => errored.child.emit("error", new Error("spawn")));
        callback();
      },
    });
    await expect(
      (await runner(errored)).run({
        agent: agent(),
        prompt: "x",
        onSpawn: vi.fn(),
        onSession: vi.fn(),
