...
function bindPidFileMatching(locator: ClaudeSessionLocator): PidFileMatching {
  return (locator as unknown as { tryPidFileMatching: PidFileMatching }).tryPidFileMatching.bind(
    locator,
  ) as PidFileMatching;
}
...
function bindProjectDir(locator: ClaudeSessionLocator): (cwd: string) => string {
  return (locator as unknown as { getProjectDir: (cwd: string) => string }).getProjectDir.bind(
    locator,
  );
}
...
describe("ClaudeSessionLocator", () => {
  ...
  it("matches resumed sessions directly and carries live PID status metadata", () => {
    ...
    fs.writeFileSync(
      path.join(sessionsDir, "123.json"),
      JSON.stringify({
        pid: 123,
        sessionId,
        cwd,
  ...
  describe("discoverLiveSessions", () => {
    ...
    it("should scan only directories matching process CWDs", () => {
      ...
      const processes = [{ pid: 1, command: "claude", cwd: "/my/project", tty: "" }];
  ...
  describe("tryPidFileMatching", () => {
    ...
    it("should fall back for all processes when sessions dir does not exist", () => {
      const missingSessions = bindPidFileMatching(
        new ClaudeSessionLocator({
          projectsDir,
  ...
  describe("getProjectDir", () => {
    ...
    it("should encode dots as hyphens", () => {
      expect(encode("/Users/foo/.worktrees/x")).toBe(
        path.join(projectsDir, "-Users-foo--worktrees-x"),
      );
    });
