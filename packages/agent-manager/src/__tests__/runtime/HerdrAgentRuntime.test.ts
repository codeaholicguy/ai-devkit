...
function createRunner(
  responses: Array<{ stdout?: string; stderr?: string; reject?: Error }> = [],
): HerdrCommandRunner {
...
describe("HerdrAgentRuntime", () => {
  ...
  it("rejects start responses when Herdr process info does not include a pid", async () => {
    const runner = createRunner([workspaceResponse(), agentResponse(), processInfoResponse()]);
    const runtime = new HerdrAgentRuntime({ runner, env: {} });

    await expect(runtime.startAgent(startInput)).rejects.toThrow(
      new HerdrRuntimeError("Herdr pane process-info response did not include process pid."),
    );
  });

  it("rejects start responses that do not include a pane id", async () => {
    const runner = createRunner([{ stdout: '{"result":{"workspace":{"workspace_id":"w1"}}}' }]);
    const runtime = new HerdrAgentRuntime({ runner, env: {} });

    await expect(runtime.startAgent(startInput)).rejects.toThrow(
      new HerdrRuntimeError("Herdr workspace create response did not include root pane id."),
    );
  });
