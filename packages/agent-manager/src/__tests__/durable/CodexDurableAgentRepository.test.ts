...
describe("Codex durable-agent repository binding", () => {
  it("creates Codex agents unbound and binds during the owned run", async () => {
    ...
    await expect(
      repository.bindProviderSession(agent.id, run.token, SESSION),
  ...
  it("rejects invalid, stale, replacement, and duplicate bindings", async () => {
    ...
    await expect(repository.bindProviderSession(first.id, "stale", SESSION)).rejects.toMatchObject({
      code: "DURABLE_AGENT_REPOSITORY",
    });
    await expect(
      repository.bindProviderSession(first.id, firstRun.token, "invalid"),
    ).rejects.toMatchObject({ code: "DURABLE_AGENT_REPOSITORY" });
    await repository.bindProviderSession(first.id, firstRun.token, SESSION);
    await expect(
      repository.bindProviderSession(first.id, firstRun.token, OTHER_SESSION),
    ).rejects.toMatchObject({ code: "CODEX_SESSION_MISMATCH" });
    await expect(
      repository.bindProviderSession(second.id, secondRun.token, SESSION),
    ).rejects.toMatchObject({ code: "CODEX_SESSION_MISMATCH" });
  });
});
