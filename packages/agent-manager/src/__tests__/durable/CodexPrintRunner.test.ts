...
describe("CodexPrintRunner", () => {
  ...
  it("rejects oversized output, non-zero exit, and missing process identity without leaking stderr", async () => {
    ...
    await expect(
      (await runner(oversizedWithoutNewline, 10)).run({
        agent: agent(),
        prompt: "x",
        onSpawn: vi.fn(),
        onSession: vi.fn(),
  ...
  it("classifies callback processing failure and kills when spawn persistence fails", async () => {
    ...
    await expect(
      (await runner(callbackFailure)).run({
        agent: agent(),
        prompt: "x",
        onSpawn: vi.fn(),
