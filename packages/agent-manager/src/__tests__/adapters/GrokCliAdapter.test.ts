...
describe("GrokCliAdapter", () => {
  ...
  function writeActiveSessions(
    entries: Array<{ pid: number; cwd: string; opened_at?: number }>,
  ): void {
  ...
  describe("canHandle", () => {
    ...
    it("returns true for grok with a full path and args", () => {
      expect(
        adapter.canHandle(proc({ command: "/Users/dev/.grok/bin/grok --always-approve" })),
      ).toBe(true);
    });
  ...
  describe("detectAgents", () => {
    ...
    it("picks the most recently active session dir when a cwd has several", async () => {
      ...
      writeSession({
        ...
        records: [userRecord("old one")],
        mtime: older,
      });
      writeSession({
        id: "019f0000-0000-7000-8000-00000000000b",
        records: [userRecord("newest one")],
      });
      mockedListAgentProcesses.mockReturnValue([proc()]);
  ...
  describe("getConversation", () => {
    ...
    it("excludes system records unless verbose", () => {
      ...
      expect(adapter.getConversation(dir, { verbose: true }).map((m) => m.role)).toEqual([
        "system",
        "user",
      ]);
    });
