...
describe("ClaudeCodeAdapter", () => {
  ...
  describe("detectAgents", () => {
      ...
      fs.writeFileSync(
        ...
        [
          JSON.stringify({
            ...
            cwd: "/Users/test/my-project",
            message: { content: "Investigate failing tests" },
          }),
    ...
    it("should handle process with empty cwd in process-only fallback", async () => {
      const processes: ProcessInfo[] = [{ pid: 300, command: "claude", cwd: "", tty: "ttys003" }];
      mockedListAgentProcesses.mockReturnValue(processes);
      mockedEnrichProcesses.mockReturnValue(processes);
      ...
      fs.writeFileSync(
        ...
        [
          JSON.stringify({
            ...
            cwd: "/project/resumed",
            message: { content: "resumed conversation" },
          }),
      ...
      fs.writeFileSync(
        ...
        [
          JSON.stringify({
            ...
            timestamp: new Date().toISOString(),
            cwd: "/project/direct",
            message: { content: "hello from pid file" },
          }),
          JSON.stringify({ type: "assistant", timestamp: new Date().toISOString() }),
        ].join("\n"),
      );
      ...
      fs.writeFileSync(
        ...
        [
          JSON.stringify({
            ...
            timestamp: new Date().toISOString(),
            cwd: "/project/wait",
            message: { content: "/reddit-commenter" },
          }),
          JSON.stringify({
      ...
      fs.writeFileSync(
        ...
        [
          JSON.stringify({
            ...
            timestamp: new Date().toISOString(),
            cwd: "/project/alpha",
            message: { content: "direct question" },
          }),
          JSON.stringify({ type: "assistant", timestamp: new Date().toISOString() }),
        ].join("\n"),
      );
  ...
  describe("helper methods", () => {
    describe("determineStatus", () => {
      it('should return "unknown" for sessions with no last entry type', () => {
        const determineStatus = (adapter as any).parser.determineStatus.bind(
    ...
    describe("extractUserMessageText", () => {
      it("should extract plain string content", () => {
        const extract = (adapter as any).parser["extractUserMessageText"].bind(
          (adapter as any).parser,
        );
      ...
      it("should parse command-message tags", () => {
        ...
        const msg =
          "<command-message><command-name>commit</command-name><command-args>fix bug</command-args></command-message>";
      ...
      it("should filter noise messages", () => {
        ...
        expect(
          extract("This session is being continued from a previous conversation"),
        ).toBeUndefined();
      });
    });
