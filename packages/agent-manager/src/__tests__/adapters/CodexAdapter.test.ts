...
import { captureProcessSnapshot, enrichProcesses, listAgentProcesses } from '../../utils/process.js';
import { generateAgentName, matchProcessesToSessions, type MatchResult } from '../../utils/matching.js';
...
describe('CodexAdapter', () => {
    ...
    it('exposes codex type and handles Codex executable names only', () => {
        ...
        expect(adapter.canHandle({ pid: 2, command: '/usr/local/bin/CODEX --sandbox workspace-write', cwd: '/repo', tty: 'ttys002' })).toBe(true);
    ...
    it('maps a running Codex process through legacy session matching', async () => {
        ...
        const { sessionFile, dateDir } = writeSession('sess-abc', '/repo-a', [
            { type: 'event', timestamp: new Date().toISOString(), payload: { type: 'token_count', message: 'Implement adapter flow' } },
        ]);
    ...
    it('maps resumed sessions directly by id', async () => {
        ...
        const { sessionFile } = writeSession(sessionId, '/repo-a', [
            { type: 'event', timestamp: new Date().toISOString(), payload: { type: 'agent_message', message: 'resumed codex conversation' } },
        ], '2026/06/09');
    ...
    it('uses hook session mapping before a stale registry entry for the same pid', async () => {
        ...
        fs.writeFileSync(staleSession, [
            JSON.stringify({ type: 'session_meta', payload: { id: 'stale', timestamp: new Date().toISOString(), cwd: '/repo-stale' } }),
            JSON.stringify({ type: 'event', timestamp: new Date().toISOString(), payload: { type: 'agent_message', message: 'stale' } }),
        ].join('\n'));
        registry.register(registryEntry({ sessionFilePath: staleSession }));

        const mappedAdapter = new CodexAdapter(registry);
        const { sessionFile } = writeSession('current', '/repo-current', [
            { type: 'event', timestamp: new Date().toISOString(), payload: { type: 'agent_message', message: 'Hello from current mapping' } },
        ], '2026/06/26');
        fs.mkdirSync(path.join(tmpHome, '.codex', 'ai-devkit'), { recursive: true });
        fs.writeFileSync(path.join(tmpHome, '.codex', 'ai-devkit', 'sessions.json'), JSON.stringify({ 100: sessionFile }));
    ...
    it('uses a valid registry cache entry before scanning sessions', async () => {
        ...
        const { sessionFile } = writeSession('cached', '/repo-a', [
            { type: 'event', timestamp: new Date().toISOString(), payload: { type: 'token_count', message: 'Hello from cache' } },
        ]);
    ...
    it('delegates conversation parsing and historical session listing', async () => {
        const { sessionFile } = writeSession('listed', '/repo', [
            { type: 'event', timestamp: '2025-01-01T00:00:01Z', payload: { type: 'user_message', message: 'first user' } },
            { type: 'event', timestamp: '2025-01-01T00:00:02Z', payload: { type: 'agent_message', message: 'answer' } },
        ], '2025/01/01');
    ...
    function writeSession(
        ...
        fs.writeFileSync(sessionFile, [
            JSON.stringify({ type: 'session_meta', payload: { id: sessionId, timestamp: '2026-03-18T15:00:00Z', cwd } }),
