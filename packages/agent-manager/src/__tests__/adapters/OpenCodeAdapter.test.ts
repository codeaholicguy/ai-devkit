...
function makeDb(queries: {
  ...
  const prepareImpl = (sql: string) => {
    ...
    if (
      normalized.includes("from message") &&
      !normalized.includes("'$.time.completed'") &&
      !normalized.includes("order by p.time_created")
    ) {
      return {
    ...
    if (normalized.includes("'$.time.completed'")) {
      return {
        get: () =>
          queries.lastAssistant === undefined ? undefined : (queries.lastAssistant ?? undefined),
      };
    }
    ...
    if (normalized.includes("json_extract(m.data, '$.role') = 'user'")) {
      return {
        get: () =>
          queries.firstUserText === undefined ? undefined : (queries.firstUserText ?? undefined),
      };
    }
...
describe("OpenCodeAdapter", () => {
  ...
  describe("canHandle", () => {
    it("returns true for opencode command", () => {
      expect(adapter.canHandle({ pid: 1, command: "opencode", cwd: "/repo", tty: "ttys001" })).toBe(
        true,
      );
    });
    ...
    it("returns true for full path opencode", () => {
      expect(
        adapter.canHandle({
          pid: 2,
          command: "/usr/local/bin/opencode serve",
          cwd: "/repo",
          tty: "ttys002",
        }),
      ).toBe(true);
    ...
    it("returns false for non-opencode processes", () => {
      expect(
        adapter.canHandle({ pid: 4, command: "node server.js", cwd: "/repo", tty: "ttys004" }),
      ).toBe(false);
    });
    ...
    it("returns text messages from session parts", () => {
      const db = makeDb({
        parts: [
          {
            role: "user",
            partData: JSON.stringify({ type: "text", text: "Hello agent" }),
            timeCreated: 1000,
          },
          {
            role: "assistant",
            partData: JSON.stringify({ type: "text", text: "Hi, how can I help?" }),
            timeCreated: 2000,
          },
        ],
      });
    ...
    it("skips reasoning parts when verbose is false", () => {
      const db = makeDb({
        parts: [
          {
            role: "assistant",
            partData: JSON.stringify({ type: "reasoning", reasoning: "internal thought" }),
            timeCreated: 1000,
          },
          {
            role: "assistant",
            partData: JSON.stringify({ type: "text", text: "Answer" }),
            timeCreated: 2000,
          },
        ],
      });
    ...
    it("includes reasoning and tool parts when verbose is true", () => {
      const db = makeDb({
        parts: [
          {
            role: "assistant",
            partData: JSON.stringify({ type: "reasoning", reasoning: "my thinking" }),
            timeCreated: 1000,
          },
          {
            role: "assistant",
            partData: JSON.stringify({ type: "tool", tool: "read_file" }),
            timeCreated: 2000,
          },
        ],
      });
