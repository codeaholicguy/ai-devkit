import { AgentStatus, type AgentAdapter, type AgentInfo, type AgentType, type ConversationMessage } from '../adapters/AgentAdapter.js';
import type { AgentManager } from '../AgentManager.js';
import { TtyWriter } from '../terminal/TtyWriter.js';
import type { TerminalFocusManager, TerminalLocation } from '../terminal/TerminalFocusManager.js';
...
function parseMilliseconds(value: string | undefined, defaultMs: number): { milliseconds: number; label?: string } {
  if (value === undefined) return { milliseconds: defaultMs };
  const trimmed = value.trim();
  if (!/^\d+$/.test(trimmed)) throw new Error('Expected positive integer milliseconds.');
  const milliseconds = Number(trimmed);
  if (!Number.isFinite(milliseconds) || milliseconds <= 0) {
    throw new Error('Expected positive integer milliseconds.');
  }
  return { milliseconds, label: `${milliseconds}ms` };
}

const silentReporter: SendReporter = {
  info: () => undefined,
  warning: () => undefined,
  success: () => undefined,
  error: () => undefined,
