...
async function builtInSkillsCheck(
  agent: ReadinessAgentType,
  rt: Runtime,
  ...
  const checks = await Promise.all(
    rt.builtInSkillNames.map(async (name) => ({
      name,
...
async function codexIntegrationCheck(rt: Runtime): Promise<IntegrationReadinessCheck> {
  ...
  const mappingFile = await mappingCheck(
    join(rt.homeDir, ".codex", "ai-devkit", "sessions.json"),
    rt,
  );
...
async function authCheck(
  agent: ReadinessAgentType,
  rt: Runtime,
): Promise<AuthReadinessCheck | undefined> {
...
async function integrationCheck(
  agent: ReadinessAgentType,
  rt: Runtime,
): Promise<IntegrationReadinessCheck | undefined> {
...
export async function getAgentReadinessReports(
  ...
  const entries = await Promise.all(
    READINESS_AGENT_TYPES.map(async (agent) => [agent, await agentReadiness(agent, rt)] as const),
  );
