...
interface RunnerLike {
  run(request: Parameters<ClaudePrintRunner["run"]>[0]): Promise<ClaudePrintRunResult>;
}
...
export class ClaudePrintAgentService {
  ...
  async send(reference: string, prompt: string): Promise<ClaudePrintSendResult> {
    const completed = await runDurableAgent({
      ...
      ambiguousError: () =>
        new ClaudePrintError(
          `Multiple durable agents match "${reference}".`,
          "DURABLE_AGENT_AMBIGUOUS",
        ),
      providerError: () =>
        new ClaudePrintError("Durable agent provider is not Claude.", "CLAUDE_PRINT_UNSUPPORTED"),
      execute: (agent, token) =>
        this.runner.run({
          agent,
          prompt,
      ...
      isSessionMismatch: (error) =>
        error instanceof ClaudePrintError && error.code === "CLAUDE_SESSION_MISMATCH",
    });
