...
export class ClaudeStreamParser {
    ...
    this.lines = new LineBuffer(
      ...
      () =>
        new ClaudePrintError(
          "Claude stream line exceeded the safety limit.",
          "CLAUDE_STREAM_OVERSIZED",
        ),
    );
  }
  ...
  fail(error: unknown): void {
    this.protocolError =
      error instanceof ClaudePrintError
        ? error
  ...
  result(close: ChildCloseResult): ClaudePrintRunResult {
    ...
    this.lines.assertComplete(
      () =>
        new ClaudePrintError("Claude stream ended with incomplete JSON.", "CLAUDE_STREAM_INVALID"),
    );
    if (close.code !== 0) {
  ...
  private processLine(line: Buffer): void {
    ...
    if (typeof event.session_id === "string" && event.session_id !== this.expectedSessionId) {
      throw new ClaudePrintError(
        "Claude returned a different session identity.",
  ...
  private parseEvent(line: Buffer): Record<string, unknown> {
    ...
    if (!value || typeof value !== "object" || Array.isArray(value)) {
      throw new ClaudePrintError(
        "Claude emitted a non-object stream message.",
  ...
  private recordTerminalResult(event: Record<string, unknown>): void {
    ...
    if (typeof event.session_id !== "string" || typeof event.result !== "string") {
      throw new ClaudePrintError(
        "Claude emitted an invalid terminal result.",
        "CLAUDE_STREAM_INVALID",
      );
    }
