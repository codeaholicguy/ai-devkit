...
export class ClaudeSessionParser {
  ...
  determineStatus(session: ClaudeSession): AgentStatus {
    ...
    if (session.lastEntryType === "user") {
      return session.isInterrupted ? AgentStatus.WAITING : AgentStatus.RUNNING;
    }

    if (session.lastEntryType === "progress" || session.lastEntryType === "thinking") {
  ...
  private extractConversationContent(
    ...
    for (const block of content) {
      if (block.type === "text" && block.text?.trim()) {
        ...
        const inputSummary =
          block.input?.file_path || block.input?.pattern || block.input?.command || "";
