...
export class CodexAdapter implements AgentAdapter {
    ...
    async detectAgents(context?: AgentDetectionContext): Promise<AgentInfo[]> {
        ...
        const legacyResult = this.mapLegacyMatches(locatorResult.legacyMatches, locatorResult.contentCache);
    ...
    private mapDirectMatches(matches: CodexDirectMatch[]): MappedAgentResult {
        ...
        for (const match of matches) {
            ...
            if (session) {
                agents.push(this.mapper.mapSessionToAgent(session, match.process, match.sessionFile.filePath));
            } else {
                fallback.push(match.process);
            }
        }
    ...
    private mapLegacyMatches(
        ...
        for (const match of matches) {
            const session = this.parser.readSession(match.session.filePath, contentCache.get(match.session.filePath));
            if (session) {
