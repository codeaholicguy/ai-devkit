...
interface ProbeLike {
  validate(): Promise<{ executable: string; version: string }>;
}
interface RunnerLike {
  run(request: Parameters<CodexPrintRunner["run"]>[0]): Promise<CodexPrintRunResult>;
}
...
export class CodexPrintAgentService {
  ...
  async send(reference: string, prompt: string): Promise<CodexPrintSendResult> {
    const completed = await runDurableAgent({
      ...
      ambiguousError: () =>
        new CodexPrintError("Multiple print agents match.", "CODEX_UNSUPPORTED"),
      providerError: () =>
        new CodexPrintError("Print agent provider is not Codex.", "CODEX_UNSUPPORTED"),
      execute: (agent, token) =>
        this.runner.run({
          agent,
          prompt,
          ...
          onSession: async (sessionId) => {
            await this.repository.bindProviderSession(agent.id, token, sessionId);
          },
        }),
      isSessionMismatch: (error) =>
        error instanceof CodexPrintError && error.code === "CODEX_SESSION_MISMATCH",
    });
