...
export class CodexStreamParser {
  ...
  result(close: ChildCloseResult): CodexPrintRunResult {
    ...
    if (close.code !== 0) {
      throw new CodexPrintError(
        `Codex print run failed${close.signal ? ` (${close.signal})` : "."}`,
        "CODEX_PROCESS",
      );
    }
    if (this.sessionId === null)
      throw new CodexPrintError("Codex stream ended without a thread identity.", "CODEX_PROTOCOL");
    if (!this.turnCompleted)
      throw new CodexPrintError("Codex stream ended before turn completion.", "CODEX_PROTOCOL");
    if (this.messages.length === 0)
      throw new CodexPrintError(
        "Codex stream ended without an assistant result.",
  ...
  private async recordSession(event: Record<string, unknown>): Promise<void> {
    ...
    if (this.expectedSessionId !== null && this.expectedSessionId !== sessionId) {
      throw new CodexPrintError(
        "Codex returned a different session identity.",
