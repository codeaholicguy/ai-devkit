...
function printSummary(
  ...
  console.log(
    `| Cold Start             | ${fmt(coldStart.latencyMs, "ms")}     | < ${SLA.COLD_START_MS}ms        | ${pass(coldStart.passed)} |`,
  );
  console.log(
    `| Store Latency (avg)    | ${fmt(storeResult.avgTimeMs, "ms")}     | < ${SLA.STORE_LATENCY_MS}ms         | ${pass(storeResult.slaPassed)} |`,
  );
  console.log(
    `| Store Latency (P99)    | ${fmt(storeResult.p99Ms, "ms")}     | < ${SLA.STORE_LATENCY_MS}ms         | ${pass(storeResult.p99Ms < SLA.STORE_LATENCY_MS)} |`,
  );
  console.log(
    `| Search Latency (avg)   | ${fmt(searchResult.avgTimeMs, "ms")}     | < ${SLA.SEARCH_LATENCY_MS}ms          | ${pass(searchResult.slaPassed)} |`,
  );
  console.log(
    `| Search Latency (P99)   | ${fmt(searchResult.p99Ms, "ms")}     | < ${SLA.SEARCH_LATENCY_MS}ms          | ${pass(searchResult.p99Ms < SLA.SEARCH_LATENCY_MS)} |`,
  );
  console.log(
    `| Mixed Store (P99)      | ${fmt(mixedResult.storeResult.p99Ms, "ms")}     | < ${SLA.STORE_LATENCY_MS}ms         | ${pass(mixedResult.storeResult.p99Ms < SLA.STORE_LATENCY_MS)} |`,
  );
  console.log(
    `| Mixed Search (P99)     | ${fmt(mixedResult.searchResult.p99Ms, "ms")}     | < ${SLA.SEARCH_LATENCY_MS}ms          | ${pass(mixedResult.searchResult.p99Ms < SLA.SEARCH_LATENCY_MS)} |`,
  );
  console.log(
    `| Memory Usage           | ${fmt(memoryUsageMB, "MB")}     | < ${SLA.MEMORY_FOOTPRINT_MB}MB        | ${pass(memoryPassed)} |`,
  );
  console.log(
    `| Database Size          | ${fmt(dbSizeMB, "MB")}     | < ${expectedDbSizeMB.toFixed(0)}MB          | ${pass(dbSizePassed)} |`,
  );
...
async function main(): Promise<void> {
  ...
  try {
    ...
    console.log(
      `  Store avg/P99:      ${mixedResult.storeResult.avgTimeMs.toFixed(2)}ms / ${mixedResult.storeResult.p99Ms.toFixed(2)}ms`,
    );
    console.log(
      `  Search avg/P99:     ${mixedResult.searchResult.avgTimeMs.toFixed(2)}ms / ${mixedResult.searchResult.p99Ms.toFixed(2)}ms`,
    );
    ...
    console.log(
      `  Memory (heap used): ${memoryUsageMB.toFixed(2)} MB (SLA: < ${SLA.MEMORY_FOOTPRINT_MB} MB)`,
    );
    console.log(
      `  Database size:      ${formatBytes(dbSizeBytes)} (SLA: < ${SLA.DATABASE_SIZE_MB_PER_10K} MB per 10K items)`,
    );
