...
import {
  ...
  StorageError,
  ValidationError,
} from "../domain/knowledge/errors.js";
...
import {
  ...
  normalizeTags,
  normalizeTitle,
} from "../domain/knowledge/normalize.js";
  ...
  try {
    return db.transaction(() => {
      ...
      updateKnowledgeRow(db, {
        ...
        embeddingVersion:
          embedding?.version ??
          (invalidatesEmbedding ? null : (existing.embedding_version ?? null)),
      });
...
function buildRecencyCounts(rows: KnowledgeRow[]): Array<{ bucket: string; count: number }> {
  const counts = new Map<RecencyBucket, number>(RECENCY_BUCKETS.map((bucket) => [bucket, 0]));
  const now = Date.now();
