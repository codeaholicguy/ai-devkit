...
export async function searchKnowledgeHybrid(
  ...
  if (count > MAX_SEMANTIC_CORPUS) {
    return {
      ...
      semantic: semanticStatus(
        "corpus-too-large",
        count,
        `Semantic scan is limited to ${MAX_SEMANTIC_CORPUS} rows`,
      ),
    };
  }
  ...
  try {
    ...
    if (queryEmbedding.length !== MODEL_DIMENSION) {
      throw new Error(
        `Semantic model returned ${queryEmbedding.length} dimensions; expected ${MODEL_DIMENSION}`,
      );
    }
    ...
    for (const row of rows) {
      try {
        ...
        try {
          ...
          /* malformed legacy tags rank without tags */
        }
        candidates.push({
          id: row.id,
          title: row.title,
    ...
    return {
      ...
      totalMatches: new Set([
        ...lexical.results.map((item) => item.id),
...
export async function storeKnowledgeSemantic(
  ...
  try {
    ...
    const vector = await embedder.embed(
      buildEmbeddingText({ title: input.title, content: input.content, tags }),
    );
  ...
  for (let offset = 0; offset < pending.length; offset += batchSize) {
    ...
    try {
      ...
      if (vectors.length !== batch.length)
        throw new Error("Semantic model returned the wrong batch size");
      db.transaction(() => {
