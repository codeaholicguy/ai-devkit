import {
  ...
  validateStoreInput,
  validateUpdateInput,
} from "../../../../src/domain/knowledge/validate";
...
describe("validator", () => {
  ...
  describe("validateContent", () => {
    const validContent =
      "This is valid content that is at least fifty characters long for testing purposes.";
    ...
    it("should not throw for valid input with content", () => {
      expect(() =>
        validateUpdateInput({
          id: "some-valid-uuid",
          content:
            "Valid content that is long enough to pass the minimum content length validation requirement.",
        }),
      ).not.toThrow();
    });
  ...
  describe("validateStoreInput", () => {
    const validInput = {
      ...
      content:
        "When building REST APIs, always use Response DTOs instead of returning domain entities directly. This provides better API versioning and security.",
      tags: ["api", "backend"],
      scope: "global",
    };
