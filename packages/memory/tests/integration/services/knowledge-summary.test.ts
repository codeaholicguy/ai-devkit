...
describe("knowledge summary command", () => {
  ...
  it("counts scopes, tags, and recency buckets", () => {
    memoryStoreCommand({
      ...
      content:
        "The memory dashboard should expose a local read-only view for inspecting global memory records safely.",
      tags: "dashboard,read",
      scope: "global",
    });
    memoryStoreCommand({
      ...
      content:
        "The dashboard plugin should reuse the plugin runtime and configured memory database path.",
