...
describe("update handler", () => {
  ...
  const validInput = {
    ...
    content:
      "When building REST APIs, always use Response DTOs instead of returning domain entities directly. This provides better API versioning.",
    tags: ["api", "backend"],
    scope: "global",
  };
  ...
  const validInput2 = {
    ...
    content:
      "Always inject dependencies via constructor to enable easier testing and loose coupling between components in the system.",
  ...
  describe("successful updates", () => {
    ...
    it("should update content only", () => {
      const stored = storeKnowledge(validInput);
      const newContent =
        "Updated content that is long enough to pass validation rules. This replaces the old content entirely with new information.";
      const result = updateKnowledge({
        id: stored.id,
        content: newContent,
      });
    ...
    it("should update multiple fields at once", () => {
      ...
      const newContent =
        "Completely new content for this knowledge item that is long enough to pass the validation rules set in the validator.";
      const result = updateKnowledge({
        id: stored.id,
        title: newTitle,
        content: newContent,
    ...
    it("should refresh updated_at timestamp", () => {
      const stored = storeKnowledge(validInput);
      const rowBefore = db.queryOne<KnowledgeRow>("SELECT * FROM knowledge WHERE id = ?", [
        stored.id,
      ]);
      ...
      vi.spyOn(global, "Date").mockImplementation(function () {
        return futureDate as unknown as Date;
      });
      ...
      const rowAfter = db.queryOne<KnowledgeRow>("SELECT * FROM knowledge WHERE id = ?", [
        stored.id,
      ]);
    ...
    it("should recalculate content_hash when content changes", () => {
      ...
      updateKnowledge({
        id: stored.id,
        content:
          "Completely different content that should produce a different hash value when processed by the normalizer and hasher.",
      });
