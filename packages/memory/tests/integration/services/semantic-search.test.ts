...
import { updateKnowledge } from "../../../src/services/knowledge.service";
import { searchKnowledgeHybrid } from "../../../src/services/semantic.service";
import { MODEL_VERSION } from "../../../src/semantic/model";
import { serializeEmbedding } from "../../../src/semantic/embeddings";
import { type LocalEmbedder } from "../../../src/semantic/embedder";
import {
  getSemanticStatus,
  reembedKnowledge,
  storeKnowledgeSemantic,
} from "../../../src/services/semantic.service";
...
describe("semantic search integration", () => {
  ...
  it("retrieves a semantic-only paraphrase and exposes explanations", async () => {
    const relevant = store(
      ...
      "Public HTTP payloads are mapped through dedicated transfer objects before leaving the service boundary.",
    );
    const distractor = store(
      "Queue retention policy rules",
      "Failed queue messages remain available for seven days before automated cleanup removes them permanently.",
    );
    const db = getDatabase();
    db.execute("UPDATE knowledge SET embedding = ?, embedding_version = ? WHERE id = ?", [
      serializeEmbedding(first),
      MODEL_VERSION,
      relevant,
    ]);
    db.execute("UPDATE knowledge SET embedding = ?, embedding_version = ? WHERE id = ?", [
      serializeEmbedding(second),
      MODEL_VERSION,
      distractor,
    ]);
  ...
  it("degrades to lexical results when the local model is unavailable", async () => {
    const relevant = store(
      "Exact retry policy identifier",
      "The WEB-1842 retry rule applies exponential backoff only to transient server failures.",
    );
  ...
  it("invalidates embeddings for document changes but retains them for scope-only updates", () => {
    const id = store(
      ...
      "Changing searchable memory text must invalidate its previously computed semantic embedding value.",
    );
    const db = getDatabase();
    db.execute("UPDATE knowledge SET embedding = ?, embedding_version = ? WHERE id = ?", [
      serializeEmbedding(first),
    ...
    expect(
      db.queryOne<{ embedding: Buffer | null }>("SELECT embedding FROM knowledge WHERE id = ?", [
        id,
      ])?.embedding,
    ).not.toBeNull();
    ...
    expect(
      db.queryOne<{ embedding: Buffer | null; embedding_version: string | null }>(
        "SELECT embedding, embedding_version FROM knowledge WHERE id = ?",
        [id],
      ),
  ...
  it("backfills missing embeddings and skips current rows on the next run", async () => {
    store(
      "Missing semantic vector",
      "This memory begins without an embedding and must be included by the resumable backfill operation.",
    );
    store(
      "Another missing vector",
      "This second memory also needs a deterministic semantic vector written by the maintenance command.",
    );
    ...
    expect(
      getDatabase().queryOne<{ count: number }>(
        "SELECT COUNT(*) AS count FROM knowledge WHERE embedding_version = ?",
        [MODEL_VERSION],
      )?.count,
    ).toBe(2);
  });
  ...
  it("stores an embedding with semantic-aware writes", async () => {
    const result = await storeKnowledgeSemantic(
      {
        ...
        content:
          "Semantic-enabled stores write the searchable memory and its matching model vector in one database transaction.",
        tags: ["write"],
      },
      { embedder },
    );
    ...
    expect(
      getDatabase().queryOne<{ embedding: Buffer; embedding_version: string }>(
        "SELECT embedding, embedding_version FROM knowledge WHERE id = ?",
        [result.id],
      ),
  ...
  it("reports model and embedding readiness without loading the runtime", async () => {
    store(
      "Status missing vector",
      "The status command reports memories that have not yet received a compatible semantic embedding value.",
    );
    const status = await getSemanticStatus({
      modelsRoot: join(tmpdir(), "definitely-missing-model-root"),
    });
  ...
  it("ignores corrupt vectors without losing lexical results", async () => {
    const relevant = store(
      "Corrupt vector fallback",
      "Lexical retrieval remains available when a stored semantic vector has an invalid byte length.",
    );
    getDatabase().execute(
      "UPDATE knowledge SET embedding = ?, embedding_version = ? WHERE id = ?",
      [Buffer.from([1, 2]), MODEL_VERSION, relevant],
    );

    const result = await searchKnowledgeHybrid(
      { query: "corrupt vector fallback", limit: 5 },
      { embedder },
    );
  ...
  it("skips semantic inference when the eligible corpus exceeds the scan limit", async () => {
    ...
    db.instance.transaction(() => {
      for (let index = 0; index < 5_001; index++) {
        insert.run(
          `large-${index}`,
          `Large corpus memory ${index}`,
          `Large corpus content ${index} contains enough searchable developer knowledge for validation.`,
          "[]",
          "global",
          `large corpus memory ${index}`,
          `large-hash-${index}`,
