...
import { memoryListCommand, memoryStoreCommand, type ListKnowledgeResult } from "../../../src/api";
...
describe("list knowledge command", () => {
  ...
  function seedKnowledge(): void {
    memoryStoreCommand({
      ...
      content:
        "When building REST APIs, always use Response DTOs instead of returning domain entities directly for stable contracts.",
    ...
    memoryStoreCommand({
      ...
      content:
        "The memory dashboard graph should connect memory items to tag and scope nodes for lightweight local inspection.",
    ...
    memoryStoreCommand({
      ...
      content:
        "Plugin command entrypoints should export register and use built JavaScript files for runtime loading.",
      tags: "plugin,command",
