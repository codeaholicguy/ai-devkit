...
describe("store handler", () => {
  ...
  describe("successful storage", () => {
    ...
    it("should persist knowledge in database", () => {
      const result = storeKnowledge(validInput);
      const stored = db.queryOne<{ title: string }>("SELECT title FROM knowledge WHERE id = ?", [
        result.id,
      ]);
    ...
    it("should normalize and store tags as JSON", () => {
      ...
      const stored = db.queryOne<{ tags: string }>("SELECT tags FROM knowledge WHERE id = ?", [
        result.id,
      ]);
    ...
    it("should use global scope by default", () => {
      ...
      const stored = db.queryOne<{ scope: string }>("SELECT scope FROM knowledge WHERE id = ?", [
        result.id,
      ]);
  ...
  describe("validation errors", () => {
    ...
    it("should reject short content", () => {
      expect(() => storeKnowledge({ ...validInput, content: "Too short" })).toThrow(
        ValidationError,
      );
    });
  });
  ...
  describe("duplicate detection", () => {
    it("should reject duplicate title in same scope", () => {
      ...
      expect(() =>
        storeKnowledge({
          ...validInput,
          content:
            "This is completely different content that should still trigger a duplicate title error in the same scope.",
        }),
    ...
    it("should reject duplicate content in same scope", () => {
      storeKnowledge(validInput);
      expect(() =>
        storeKnowledge({ ...validInput, title: "Different title that is long enough" }),
      ).toThrow(DuplicateError);
    });
