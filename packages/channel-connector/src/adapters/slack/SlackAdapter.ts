...
export class SlackAdapter implements InteractiveChannelAdapter {
  ...
  constructor(
    ...
    this.web =
      dependencies.webClient ?? (new WebClient(config.botToken) as unknown as WebClientLike);
  ...
  private isValidDirectMessage(
    body: NonNullable<SlackEventEnvelope["body"]>,
    event: Record<string, unknown>,
  ): boolean {
    return (
      body.team_id === this.config.workspaceId &&
  ...
  private async handleInteraction(envelope: SlackInteractionEnvelope): Promise<void> {
    ...
    if (
      ...
      this.recentEventIds.has(`interaction:${interactionId}`)
    )
      return;
