...
function getMockBot() {
  return (
    telegrafModule as unknown as {
      __mockBot: ReturnType<typeof vi.fn>["mock"] & Record<string, unknown>;
    }
  ).__mockBot;
}
...
function expectHtmlChunkWithinLimit(chunk: string): void {
  ...
  expect((chunk.match(/<code(?: class="language-[^"]+")?>/g) ?? []).length).toBe(
...
describe("TelegramAdapter", () => {
  ...
  describe("sendMessage", () => {
    ...
    it("should split long paragraphs at readable markdown boundaries", async () => {
      const bot = getMockBot();
      const sentence =
        "This is a long sentence with enough words to make chunking choose sentence boundaries. ";
    ...
    it("should send plain text when markdown formatting throws", async () => {
      const bot = getMockBot();
      const formatter = vi
        .spyOn(telegramHtml, "markdownToTelegramHtml")
  ...
  describe("editInlineKeyboard", () => {
    ...
    it("passes undefined reply_markup to remove the keyboard", async () => {
      ...
      expect(bot.telegram.editMessageReplyMarkup).toHaveBeenCalledWith(
        "12345",
        99,
        undefined,
        undefined,
