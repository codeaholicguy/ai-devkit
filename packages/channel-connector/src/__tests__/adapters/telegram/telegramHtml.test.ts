...
describe("markdownToTelegramHtml", () => {
  ...
  it("renders inline markup in the middle of list item text", () => {
    const out = markdownToTelegramHtml("- While editing, `/x` is a **literal** character.");

    expect(out).toContain("• While editing, <code>/x</code> is a <b>literal</b> character.");
  });
  ...
  it("renders links as HTML inside list items", () => {
    ...
    expect(out).toContain('• See <a href="https://example.com?a=1&amp;b=2">docs</a>.');
