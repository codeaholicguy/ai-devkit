...
describe("SlackAdapter", () => {
  ...
  it("rejects incomplete Slack credential identity", async () => {
    ...
    await expect(validateSlackCredentials("xoxb-fake", { authTest })).rejects.toThrow(
      "Slack bot token returned incomplete identity",
    );
  });
  ...
  it("rejects an app token that cannot open Socket Mode", async () => {
    ...
    await expect(validateSlackAppToken("xapp-fake", { open })).rejects.toThrow(
      "Slack app token cannot open Socket Mode",
    );
  });
  ...
  it("acknowledges before delivering one normalized workspace DM event", async () => {
    ...
    await adapter.start();
    const item = envelope();
    item.ack.mockImplementation(async () => {
      order.push("ack");
    });
  ...
  it("sends an accessible Block Kit question and finalizes its actions", async () => {
    ...
    const adapter = new SlackAdapter(config, {
      socketClient: socket,
      webClient: { chat: { postMessage, update } },
    });
    await expect(
      adapter.sendQuestion("D123", {
        id: "q1",
        header: "Scope",
        question: "Choose one",
        allowSkip: true,
  ...
  it("acknowledges malformed and wrong-workspace block actions without delivery", async () => {
    ...
    socket.emit("interactive", {
      ack,
      body: { type: "block_actions", team: { id: "T999" }, actions: [] },
    });
