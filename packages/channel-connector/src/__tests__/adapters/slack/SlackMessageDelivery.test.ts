import { SlackMessageDelivery } from "../../../adapters/slack/SlackMessageDelivery.js";

describe("SlackMessageDelivery", () => {
  it("serializes long output as a parent followed by threaded continuations", async () => {
    ...
    const queue = new SlackMessageDelivery({ postMessage }, { maxMessageLength: 20 });
  ...
  it("waits for retryAfter before retrying a rate-limited API call", async () => {
    ...
    const postMessage = vi
      ...
      .mockResolvedValue({ ok: true, ts: "200.1" });
    const sleep = vi.fn().mockResolvedValue(undefined);
    const queue = new SlackMessageDelivery({ postMessage }, { sleep });
    await expect(queue.send("D123", "hello")).resolves.toEqual({
      messageId: "200.1",
      threadId: "200.1",
    });
  ...
  it("rejects overflow while a conversation send is pending", async () => {
    ...
    const queue = new SlackMessageDelivery({ postMessage }, { maxQueueSize: 1 });
    const first = queue.send("D123", "first");
    await expect(queue.send("D123", "second")).rejects.toThrow(
      "Slack message delivery queue is full",
    );
    release();
    await first;
  });
  ...
  it("rejects empty output and responses without a timestamp", async () => {
    ...
    const queue = new SlackMessageDelivery({ postMessage });
    await expect(queue.send("D123", "")).rejects.toThrow("Cannot send an empty Slack message");
    await expect(queue.send("D123", "hello")).rejects.toThrow(
  ...
  it("allows separate conversation queues to progress independently", async () => {
    const postMessage = vi
      .fn()
      .mockImplementation(async ({ channel }) => ({ ok: true, ts: `${channel}.1` }));
    const queue = new SlackMessageDelivery({ postMessage });
    await expect(
      Promise.all([queue.send("D1", "one"), queue.send("D2", "two")]),
