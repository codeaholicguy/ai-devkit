...
describe("standalone memory dashboard launcher", () => {
  const homeDir = join(
    tmpdir(),
    `memory-dashboard-home-${Date.now()}-${Math.random().toString(36)}`,
  );
  ...
  it("resolves configured relative memory paths from the AI DevKit config directory", async () => {
    ...
    writeFileSync(
      join(homeDir, ".ai-devkit", ".ai-devkit.json"),
      JSON.stringify({
        memory: {
    ...
    await expect(runtime.getMemoryDbPath()).resolves.toBe(
      join(homeDir, ".ai-devkit", "custom-memory.db"),
    );
  });
