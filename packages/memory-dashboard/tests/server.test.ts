...
describe("dashboard server", () => {
  const testDbPath = join(
    tmpdir(),
    `test-dashboard-server-${Date.now()}-${Math.random().toString(36)}.db`,
  );
  ...
  function seedDashboardRecords(): void {
    memoryStoreCommand({
      ...
      content:
        "The memory dashboard server should expose JSON APIs backed by the configured memory database.",
  ...
  it("returns graph nodes and edges from filtered memory records", async () => {
    memoryStoreCommand({
      ...
      content:
        "Graph data should connect memory records to their tags and scopes for the dashboard.",
